...
# ---------- Обработка ответов админа через reply (заявки и тикеты) ----------
...
async def admin_reply(message: Message):
    ...
    logging.info(f"Admin reply received: {original_msg.text[:200]}")

    # Проверяем, не является ли это ответом на тикет
    if "Новый тикет!" in original_msg.text:
        try:
            ...
            for line in lines:
                ...
                if "🎫 Номер тикета:" in line:
                    ticket_id = int(line.split(":")[1].strip())
            if not user_id or not ticket_id:
                await message.answer("Не удалось определить ID или тикет.")
                return
            ...
            try:
                ...
                pass
            await log_action(admin_id, "Ответ в тикет (через reply)", f"Тикет #{ticket_id}")
    ...
    # Парсим user_id
    try:
        user_id_match = re.search(r"User ID:\s*(\d+)", original_msg.text)
        if not user_id_match:
            await message.answer("Не удалось определить ID заявителя.")
            return
        applicant_id = int(user_id_match.group(1))
    except Exception as e:
        await message.answer(f"Ошибка парсинга ID: {e}")
        return
    ...
    # Обработка команды "принять"
    if command.startswith("принять"):
        # Убираем слово "принять" и берём остаток
        rest = command[len("принять"):].strip()
        if rest.startswith(','):
            rest = rest[1:].strip()
        ...
        if table == "flood_applications":
            ...
            if len(parts) < 2:
                await message.answer("❌ Формат: Принять,РольФлуда,РольСтаффа\nПример: Принять,Шедлетский,Модератор")
                return
            flood_role = parts[0]
            staff_role = parts[1]
            if not await check_flood_role_available(flood_role):
                ...
                return
            emoji = "🌊"  # можно улучшить, но пока так
...
async def cb_admin_app_rest(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "rest_applications", "Заявка в рест", "Формат: Принять,Роль,Время (например: Принять,Рест,7д)\nили Отказать,Причина")
...
async def cb_admin_app_staff(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "staff_applications", "Заявка в стафф", "Формат: Принять,РольФлуда,РольСтаффа (например: Принять,Шедлетский,Модератор)\nили Отказать,Причина")
