...
# ---------- Функция очистки просроченных pending ----------
async def cleanup_expired_pending():
    now = datetime.now()
    thresholds = {
        "flood": now - timedelta(days=7),
        "staff": now - timedelta(days=10),
        "rest": now - timedelta(days=30)
    }
    async with aiosqlite.connect("bot.db") as db:
        for table, threshold in thresholds.items():
            threshold_iso = threshold.isoformat()
            async with db.execute(f"SELECT user_id FROM {table}_applications WHERE status='pending' AND created_at < ?", (threshold_iso,)) as cur:
                expired_users = [row[0] for row in await cur.fetchall()]
            for user_id in expired_users:
                await db.execute(f"DELETE FROM {table}_applications WHERE user_id=?", (user_id,))
                await db.execute("UPDATE users SET status='none' WHERE user_id=?", (user_id,))
        await db.commit()
...
async def set_cooldown(user_id: int, category: str, days: int):
...
async def cb_cooldown_give_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[-1]
    await state.update_data(category=category)
    await callback.message.answer(f"Введите ID пользователя и количество дней через запятую.\nПример: 123456789,7\nДля отмены напишите 'отмена'.")
    await state.set_state(AdminCooldownGive.waiting_id_days)

@dp.message(AdminCooldownGive.waiting_id_days)
async def process_cooldown_give(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Неверный формат. Нужно: ID,количество_дней")
        return
    data = await state.get_data()
    category = data.get("category")
    await set_cooldown(user_id, category, days)
    await message.answer(f"Пользователю {user_id} выдан кулдаун на {days} дней в категории '{category}'.")
    await state.clear()
