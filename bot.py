...
async def is_user_in_application_process(state: FSMContext) -> bool:
    ...
    return current_state in [
        ...
        SupportReplyState.waiting_reply.state
    ]
...
async def admin_reply(message: Message):
    ...
    if "Новое обращение в поддержку" in original_msg.text or "Новое сообщение в диалоге поддержки" in original_msg.text:
        try:
...
async def support_reply(message: Message):
    ...
    # Если это ответ на сообщение админа - обрабатываем как ответ в диалоге
    if message.reply_to_message and "Ответ от администратора" in (message.reply_to_message.text or ""):
        # Проверяем, что ответ идет на сообщение от бота (админа)
    ...
    # Если это просто сообщение в активном диалоге (не от админа)
