...
# ---------- Закрытие тикета пользователем (ТОЛЬКО НЕ-REPLY) ----------
@dp.message(StateFilter(None), F.text, F.reply_to_message.is_(None))
async def support_close_command(message: Message):
    logger.info(f"support_close_command вызван: {message.from_user.id}, текст: {message.text}")
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await replace_menu(callback, "История очищена.", await admin_panel_keyboard(callback.from_user.id))
