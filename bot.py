...
# Функции ролей (аналогично предыдущим, но сокращённо для экономии места)
...
async def process_support(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        ...
        return
    user_id = message.from_user.id
    # Отправляем сообщение владельцам с подписью для ответа
    for admin_id in OWNERS:
        try:
            ...
            # Добавляем подпись с ID пользователя к пересланному сообщению
...
async def cancel_sent_application(message: Message):
    ...
    if await is_admin(message.from_user.id):
        return  # админ не отменяет заявки через этот обработчик
    original = message.reply_to_message
