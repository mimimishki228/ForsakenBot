...
class AdminRPReply(StatesGroup):
    waiting_action = State()

class AdminSendTemplate(StatesGroup):
    waiting_text = State()
    waiting_category = State()

# ---- Добавление/удаление МЕСТ ----
class AdminAddPlace(StatesGroup):
    waiting_flood = State()      # "Роль,🌸"
    waiting_rest = State()       # "Роль флуда,длительность"
    waiting_staff = State()      # "Роль флуда,роль"

class AdminDeletePlace(StatesGroup):
    waiting_flood = State()      # "Шедлетский,🌸" -> убрать место
    waiting_staff = State()
    waiting_rest = State()
...
async def set_user_status(user_id: int, status: str):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT INTO users (user_id, status) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET status=?",
...
async def set_info_message_id(key: str, message_id: int):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT INTO info_messages (key, message_id) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET message_id=?",
...
async def send_or_edit_info_message(key: str, text: str, parse_mode=None, entities=None):
    msg_id = await get_info_message_id(key)
    if msg_id:
        try:
            if entities:
            ...
            logger.warning(f"Не удалось отредактировать info {key}: {e}. Пересоздаю.")
            try:
                await bot.delete_message(INFO_CHANNEL_ID, int(msg_id))
            except Exception:
                pass
            async with aiosqlite.connect("bot.db") as db:
                ...
                await db.commit()
    if entities:
        msg = await bot.send_message(INFO_CHANNEL_ID, text, entities=entities)
    else:
        msg = await bot.send_message(INFO_CHANNEL_ID, text, parse_mode=parse_mode)
    await set_info_message_id(key, msg.message_id)


# ==================== Роли ====================
async def flood_role_exists(role_name: str) -> bool:
    """Проверяет, существует ли роль (независимо от занятости)."""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT 1 FROM flood_roles WHERE role_name=?", (role_name,)) as cur:
            return await cur.fetchone() is not None


async def is_flood_place_free(role_name: str, emoji: str) -> bool:
    """Проверяет, свободно ли КОНКРЕТНОЕ МЕСТО (роль+эмодзи)."""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT 1 FROM flood_roles WHERE role_name=? AND emoji=? AND user_id IS NULL",
            (role_name, emoji)
        ) as cur:
            return await cur.fetchone() is not None


async def add_flood_place(role_name: str, emoji: str) -> bool:
    """Добавляет МЕСТО (роль+эмодзи). Если роль уже есть — добавляет вариант эмодзи.
    Возвращает False, если такое место уже существует."""
    async with aiosqlite.connect("bot.db") as db:
        # Проверяем, нет ли уже такого же места
        async with db.execute(
            "SELECT 1 FROM flood_roles WHERE role_name=? AND emoji=?", (role_name, emoji)
        ) as cur:
            if await cur.fetchone():
                return False
        # Добавляем новое место (слот)
        await db.execute(
            "INSERT INTO flood_roles (role_name, emoji, user_id) VALUES (?, ?, NULL)",
            (role_name, emoji)
        )
        await db.commit()
    return True
...
async def remove_flood_place(role_name: str, emoji: str) -> bool:
    """Удаляет КОНКРЕТНОЕ место. Возвращает True, если удалено."""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id FROM flood_roles WHERE role_name=? AND emoji=?",
            (role_name, emoji)
        ) as cur:
            row = await cur.fetchone()
            if not row:
            ...
            if row[0] is not None:
                # Место занято — убираем только эмодзи (или удаляем запись?)
                # По ТЗ: "Если написать в удаление мест во флуд: Шедлетский,🌸
                # Было: Шедлетский:🌸 Стало: Шедлетский:"
                # То есть просто убираем эмодзи, оставляя роль с пустым слотом.
                await db.execute(
                    "UPDATE flood_roles SET emoji=NULL WHERE role_name=? AND emoji=?",
                    (role_name, emoji)
                )
            else:
                # Свободное место — удаляем полностью
                await db.execute(
                    "DELETE FROM flood_roles WHERE role_name=? AND emoji=? AND user_id IS NULL",
                    (role_name, emoji)
                )
        await db.commit()
    return True
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    """Назначает роль. Если место занято — освобождает его и занимает (перезапись)."""
    async with aiosqlite.connect("bot.db") as db:
        # Ищем свободное место с этой ролью и эмодзи
        async with db.execute(
            "SELECT rowid FROM flood_roles WHERE role_name=? AND emoji=? AND user_id IS NULL LIMIT 1",
            (role_name, emoji)
        ) as cur:
            row = await cur.fetchone()
        if row:
            await db.execute(
                "UPDATE flood_roles SET user_id=? WHERE rowid=?",
                (user_id, row[0])
            )
        else:
            # Если свободного нет — берём любое место с этой ролью и эмодзи (перезапись)
            async with db.execute(
                "SELECT rowid FROM flood_roles WHERE role_name=? AND emoji=? LIMIT 1",
                (role_name, emoji)
            ) as cur:
                row = await cur.fetchone()
            if row:
                # Освобождаем старого пользователя
                await db.execute(
                    "UPDATE users SET status='none' WHERE user_id=(SELECT user_id FROM flood_roles WHERE rowid=?)",
                    (row[0],)
                )
                await db.execute(
                    "UPDATE flood_roles SET user_id=? WHERE rowid=?",
...
async def revoke_flood_role(user_id: int):
    """Снимает с пользователя роль флуда, НЕ удаляя саму роль/место.
    Если эмодзи был — оставляет роль с пустым слотом."""
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "UPDATE flood_roles SET user_id=NULL WHERE user_id=?",
            (user_id,)
        )
        await db.execute("UPDATE users SET status='none' WHERE user_id=?", (user_id,))
        await db.commit()
...
async def update_flood_info():
    ...
    # Без ссылок на пользователей. Формат: "Роль:🌸" или "Роль:" или "Роль:👤"
    role_lines = []
    for role, emoji, uid in roles:
        if uid is not None and emoji:
            role_lines.append(f"{html.escape(role)}:{html.escape(emoji)}")
        elif uid is not None:
            role_lines.append(f"{html.escape(role)}:")
        else:
            role_lines.append(f"{html.escape(role)}:{html.escape(emoji or '')}")
    ...
    if template:
        ...
        final_text = "🌊 Роли флуда:\n" + roles_list

    saved = await get_info_message_entities("flood_roles")
    await send_or_edit_info_message("flood_roles", final_text, parse_mode="HTML", entities=saved)


async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    expiry_clean = expiry_date.strip()
    expiry_clean = re.sub(r'^(ДО|до)\s*', '', expiry_clean, flags=re.IGNORECASE).strip()
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT OR REPLACE INTO rest_roles (role_name, expiry_date, user_id) VALUES (?, ?, ?)",
            (role_name, expiry_clean, user_id)
        )
        await db.execute("UPDATE users SET status='rest' WHERE user_id=?", (user_id,))
        await db.commit()
    try:
        await bot.send_message(user_id, f"✅ Ваша заявка в рест одобрена! Роль: {role_name} на {expiry_clean}")
    except Exception:
        pass
...
async def update_rest_info():
    ...
    if not roles:
        ...
        return
    role_lines = []
    for role, expiry, uid in roles:
        # Без ссылок: "Роль: срок"
        role_lines.append(f"{html.escape(role)}: {html.escape(expiry)}")
    roles_list = "\n".join(role_lines)
    ...
    saved = await get_info_message_entities("rest_roles")
    await send_or_edit_info_message("rest_roles", final_text, parse_mode="HTML", entities=saved)


async def assign_staff_role(user_id: int, role_name: str, flood_role_name: str = None):
    """Назначает роль стаффа. Находит свободный слот. Проверяет лимит."""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute('SELECT role_limit FROM staff_roles WHERE role_name=? LIMIT 1', (role_name,)) as cur:
            row = await cur.fetchone()
            if not row:
                return False
            limit = row[0]
        async with db.execute(
            "SELECT COUNT(*) FROM staff_roles WHERE role_name=? AND user_id IS NOT NULL", (role_name,)
        ) as cur:
            count = (await cur.fetchone())[0]
        if count >= limit:
            return False
        async with db.execute(
            "SELECT rowid FROM staff_roles WHERE role_name=? AND user_id IS NULL LIMIT 1", (role_name,)
        ) as cur:
            row = await cur.fetchone()
            if not row:
                return False
            rowid = row[0]
        await db.execute(
            "UPDATE staff_roles SET user_id=?, username=? WHERE rowid=?",
            (user_id, flood_role_name or str(user_id), rowid)
        )
...
async def revoke_staff_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "UPDATE staff_roles SET user_id=NULL, username=NULL WHERE user_id=? AND role_name=?",
            (user_id, role_name)
        )
        await db.execute("UPDATE users SET status='none' WHERE user_id=?", (user_id,))
        await db.commit()
...
async def update_staff_info():
    async with aiosqlite.connect("bot.db") as db:
        ...
        for role_line in roles:
            ...
            try:
                ...
                continue
            async with db.execute(
                "SELECT COUNT(*) FROM staff_roles WHERE role_name=? AND user_id IS NOT NULL",
                (role_name,)
            ) as cur:
                count = (await cur.fetchone())[0]
            role_lines.append(f"{html.escape(role_name)}: {count}/{limit}")

        # {people} — список людей БЕЗ ссылок, формат "РольФлуда - РольСтаффа"
        people_lines = []
        async with db.execute(
            "SELECT role_name, username FROM staff_roles WHERE user_id IS NOT NULL ORDER BY role_name"
        ) as cur:
            occupied = await cur.fetchall()
        for role_name, flood_role in occupied:
            # username здесь — это роль флуда (сохранена при назначении)
            flood_display = flood_role if flood_role else "—"
            people_lines.append(f"{html.escape(str(flood_display))} - {html.escape(role_name)}")
    ...
    if template:
        final_text = template.replace("{roles}", top_block).replace("{people}", bottom_block)
    else:
        final_text = "🛡 Стафф:\n" + top_block
        if bottom_block:
            final_text += "\n\n" + bottom_block

    saved = await get_info_message_entities("staff_roles")
    await send_or_edit_info_message("staff_roles", final_text, parse_mode="HTML", entities=saved)


# ==================== Хранение entities ====================
async def save_info_message_entities(key: str, entities):
    import json
    if not entities:
        return
    data = json.dumps([e.model_dump() for e in entities], ensure_ascii=False)
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
            (f"entities_{key}", data)
        )
        await db.commit()


async def get_info_message_entities(key: str):
    import json
    from aiogram.types import MessageEntity
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT value FROM settings WHERE key=?", (f"entities_{key}",)) as cur:
            row = await cur.fetchone()
    ...
    try:
        ...
        logger.warning(f"Не удалось прочитать entities {key}: {e}")
        return None
...
async def cleanup_expired_pending():
    ...
    async with aiosqlite.connect("bot.db") as db:
        for table in ["flood_applications", "rest_applications", "staff_applications"]:
            async with db.execute(
                f"SELECT user_id FROM {table} WHERE status='pending' AND created_at < ?", (threshold_iso,)
            ) as cur:
...
async def cleanup_expired_bans():
    async with aiosqlite.connect("bot.db") as db:
        deleted = await db.execute(
            "DELETE FROM banned_users WHERE until_date IS NOT NULL AND until_date < datetime('now')"
        )
        await db.commit()
...
async def cleanup_expired_app_bans():
    async with aiosqlite.connect("bot.db") as db:
        deleted = await db.execute(
            "DELETE FROM application_bans WHERE until_date IS NOT NULL AND until_date < datetime('now')"
        )
        await db.commit()
...
async def is_cooldown_active(user_id: int, category: str) -> bool:
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT until_date FROM cooldowns WHERE user_id=? AND category=?", (user_id, category)
        ) as cur:
            row = await cur.fetchone()
            if row and row[0]:
...
def parse_duration(text: str) -> int:
    ...
    match = re.match(r"^(\d+)\s*([дчмсdhm])?$", text)
    if not match:
        raise ValueError("Неверный формат длительности")
    value = int(match.group(1))
    unit = match.group(2)
    if unit is None or unit in ("д", "d"):
        return value * 86400
    elif unit in ("ч", "h"):
        return value * 3600
    elif unit in ("м", "m"):
...
def format_remaining_time(until_date_str: str) -> str:
    ...
    if seconds > 0 and days == 0: parts.append(f"{seconds}с")
    return " ".join(parts) if parts else "0с"
...
async def admin_panel_keyboard(user_id: int):
    buttons = [
        ...
        [InlineKeyboardButton(text="➕ Добавить место", callback_data="admin_add_place")],
        ...
        [InlineKeyboardButton(text="🗑 Удаление мест", callback_data="admin_delete_places")],
        ...
        [InlineKeyboardButton(text="📤 Отправить шаблон", callback_data="admin_send_template")],
    ...
    if user_id == SPECIAL_OWNER:
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT value FROM settings WHERE key='ghost_mode'") as cur:
                row = await cur.fetchone()
                ghost_mode = row[0] if row else '0'
        if ghost_mode == '0':
...
def delete_places_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Роли (флуд)", callback_data="delete_flood_place")],
        [InlineKeyboardButton(text="Стафф", callback_data="delete_staff_place")],
        [InlineKeyboardButton(text="Ресты", callback_data="delete_rest_place")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="panel_admin")]
    ])
...
def send_template_category_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌊 Флуд", callback_data="send_tmpl_flood")],
        [InlineKeyboardButton(text="🍽 Рест", callback_data="send_tmpl_rest")],
        [InlineKeyboardButton(text="🛡 Стафф", callback_data="send_tmpl_staff")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="panel_admin")]
    ])
...
def add_place_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌊 Во флуд", callback_data="add_place_flood")],
        [InlineKeyboardButton(text="🛡 В стафф", callback_data="add_place_staff")],
        [InlineKeyboardButton(text="🍽 В рест", callback_data="add_place_rest")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="panel_admin")]
    ])
...
async def cmd_start(message: Message):
    ...
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT OR IGNORE INTO users (user_id, status, was_in_group, in_group) VALUES (?, 'none', 0, 0)",
            (user_id,)
        )
        await db.commit()
    status = await get_user_status(user_id)
...
# ==================== Ghost mode ====================
@dp.message(Command("ghost"))
async def cmd_ghost(message: Message):
    user_id = message.from_user.id
    if user_id != SPECIAL_OWNER:
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('ghost_mode', ?)", (new_mode,))
        await db.commit()
...
async def cb_admin_logs(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id, action, details, timestamp FROM logs ORDER BY timestamp DESC LIMIT 100"
        ) as cur:
            rows = await cur.fetchall()
    if not rows:
        await callback.message.edit_text("📋 Логов нет.")
        return
    lines = [f"[{ts[:19]}] Пользователь {uid}: {action} {details}" for uid, action, details, ts in rows]
...
async def app_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            f"INSERT OR REPLACE INTO {table} (user_id, text, status, created_at) VALUES (?, ?, 'pending', ?)",
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT 1 FROM support_tickets WHERE user_id=? AND status='active'", (user_id,)
        ) as cur:
            if await cur.fetchone():
...
async def process_support_start(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        cursor = await db.execute(
            "INSERT INTO support_tickets (user_id, created_at, status) VALUES (?, ?, 'active')",
            (user_id, created_at)
        )
        ticket_id = cursor.lastrowid
        await db.execute(
            "INSERT INTO support_messages (ticket_id, sender_id, text, timestamp, is_from_admin) VALUES (?, ?, ?, ?, 0)",
...
async def cb_user_ticket(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT ticket_id FROM support_tickets WHERE user_id=? AND status='active'", (user_id,)
        ) as cur:
            ...
            ticket_id = ticket[0]
        async with db.execute(
            "SELECT sender_id, text, timestamp, is_from_admin FROM support_messages WHERE ticket_id=? ORDER BY timestamp",
            (ticket_id,)
        ) as cur:
...
async def cb_admin_tickets(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT ticket_id, user_id, created_at FROM support_tickets WHERE status='active' ORDER BY created_at DESC"
        ) as cur:
...
async def process_ticket_reply(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id FROM support_tickets WHERE ticket_id=? AND status='active'", (ticket_id,)
        ) as cur:
            ticket = await cur.fetchone()
            if not ticket:
            ...
            user_id = ticket[0]
        await db.execute(
            "INSERT INTO support_messages (ticket_id, sender_id, text, timestamp, is_from_admin) VALUES (?, ?, ?, ?, 1)",
    ...
    try:
        ...
        if cmd_lower.startswith("принять"):
            ...
            if category == "flood":
                ...
                role_name = params[0]
                emoji = params[1]
                # Роль флуда может быть занята — это нормально, перезаписываем место
                ...
                role_name = params[0]
                expiry = params[1]
                expiry_clean = re.sub(r'^(ДО|до)\s*', '', expiry, flags=re.IGNORECASE).strip()
                await assign_rest_role(applicant_id, role_name, expiry_clean)
                new_status = "approved"
                result_text = f"✅ Ваша заявка в рест одобрена!\nРоль: {role_name} на {expiry_clean}"
                ...
                flood_role = params[0]
                staff_role = params[1]
                # Проверяем, существует ли роль флуда (занятость — ОК, перезаписываем)
                if not await flood_role_exists(flood_role):
                    await message.answer(f"❌ Роль флуда '{flood_role}' не существует.")
                    return
                # Назначаем роль флуда (перезапись, если занято)
                await assign_flood_role(applicant_id, flood_role, "🌊")
                # Назначаем роль стаффа (ищем свободный слот)
                ok = await assign_staff_role(applicant_id, staff_role, flood_role)
                if not ok:
                    await revoke_flood_role(applicant_id)
                    await message.answer("❌ Не удалось назначить роль стаффа (нет свободных слотов).")
                    return
                new_status = "approved"
...
async def cb_my_cooldowns(callback: CallbackQuery):
    ...
    for cat in categories:
        ...
        if active:
            async with aiosqlite.connect("bot.db") as db:
                async with db.execute(
                    "SELECT until_date FROM cooldowns WHERE user_id=? AND category=?", (user_id, cat)
                ) as cur:
                    row = await cur.fetchone()
                    until = datetime.fromisoformat(row[0])
...
async def show_applications(chat_id: int, table: str, title: str, format_hint: str):
    ...
    if not apps:
        ...
        for user_id, app_text, status in apps:
            ...
            text = (
                f"📩 {title}\nUser ID: {user_id}\nСтатус: {status_label}\nАнкета: {app_text}\n\n"
                f"Ответьте на это сообщение командой (reply).\n\n{format_hint}"
            )
...
async def cb_admin_app_flood(callback: CallbackQuery):
    ...
    await show_applications(
        callback.message.chat.id, "flood_applications", "Заявка во флуд",
        "Формат: Принять,Роль,Эмодзи\nили Отказать,Причина"
    )
...
async def cb_admin_app_rest(callback: CallbackQuery):
    ...
    if not await is_admin(user_id):
        ...
        return
    await show_applications(
        callback.message.chat.id, "rest_applications", "Заявка в рест",
        "Формат: Принять,Роль,Время\nили Отказать,Причина"
    )
...
async def cb_admin_app_staff(callback: CallbackQuery):
    ...
    if not await is_admin(user_id):
        ...
        return
    await show_applications(
        callback.message.chat.id, "staff_applications", "Заявка в стафф",
        "Формат: Принять,РольФлуда,РольСтаффа\nили Отказать,Причина"
    )
...
async def cb_admin_active_app_bans(callback: CallbackQuery):
    ...
    for uid, category, until_date in bans:
        ...
        category_names = {"flood": "флуд", "rest": "рест", "staff": "стафф", "support": "поддержка"}
...
async def toggle_app_setting(setting_key: str, info_key: str, text_on: str, text_off: str):
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT value FROM settings WHERE key=?", (setting_key,)) as cur:
            row = await cur.fetchone()
            current = row[0] if row else '1'
        new = '0' if current == '1' else '1'
        await db.execute("UPDATE settings SET value=? WHERE key=?", (new, setting_key))
        await db.commit()
...
async def cb_admin_settings_flood(callback: CallbackQuery, state: FSMContext):
    ...
    if not await is_owner(user_id):
        ...
        return
    await callback.message.answer(
        "Введите список ролей (каждая с новой строки) в формате: Роль:Эмодзи\n"
        "Для отмены напишите 'отмена'."
    )
...
async def cb_admin_settings_staff(callback: CallbackQuery, state: FSMContext):
    ...
    if not await is_owner(user_id):
        ...
        return
    await callback.message.answer(
        "Введите роли стаффа (каждая с новой строки) в формате: Название:лимит\n"
        "Для отмены напишите 'отмена'."
    )
...
# ==================== ДОБАВИТЬ МЕСТО ====================
@dp.callback_query(F.data == "admin_add_place")
async def cb_admin_add_place(callback: CallbackQuery):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await replace_menu(callback, "Куда добавить место?", add_place_menu_keyboard())


@dp.callback_query(F.data == "add_place_flood")
async def cb_add_place_flood(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.answer(
        "Отправьте: Роль,🌸.\n"
        "Пример: Шедлетский,🌸\n"
        "Если роль уже существует, будет добавлено новое место с этим эмодзи.\n"
        "Для отмены напишите 'отмена'."
    )
    await state.set_state(AdminAddPlace.waiting_flood)


@dp.message(AdminAddPlace.waiting_flood)
async def process_add_place_flood(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    if len(parts) < 2:
        await message.answer("❌ Неверный формат. Нужно: Роль,🌸")
        return
    role_name, emoji = parts[0], parts[1]
    ok = await add_flood_place(role_name, emoji)
    if not ok:
        await message.answer(f"❌ Место '{role_name}:{emoji}' уже существует.")
        await state.clear()
        return
    await log_action(user_id, "Добавление места флуда", f"{role_name}:{emoji}")
    await update_flood_info()
    await message.answer(f"✅ Место '{role_name}:{emoji}' добавлено.")
    await state.clear()


@dp.callback_query(F.data == "add_place_rest")
async def cb_add_place_rest(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.answer(
        "Отправьте: Роль флуда,длительность.\n"
        "Пример: Рест,7д\n"
        "Для отмены напишите 'отмена'."
    )
    await state.set_state(AdminAddPlace.waiting_rest)


@dp.message(AdminAddPlace.waiting_rest)
async def process_add_place_rest(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    parts = [p.strip() for p in message.text.split(',')]
    if len(parts) < 2:
        await message.answer("❌ Неверный формат. Нужно: Роль флуда,длительность")
        return
    role_name, duration = parts[0], parts[1]
    duration_clean = re.sub(r'^(ДО|до)\s*', '', duration, flags=re.IGNORECASE).strip()
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT OR REPLACE INTO rest_roles (role_name, expiry_date, user_id) VALUES (?, ?, NULL)",
            (role_name, duration_clean)
        )
        await db.commit()
    await log_action(user_id, "Добавление рест-роли", f"{role_name}:{duration_clean}")
    await update_rest_info()
    await message.answer(f"✅ Рест '{role_name}' на {duration_clean} добавлен.")
    await state.clear()


@dp.callback_query(F.data == "add_place_staff")
async def cb_add_place_staff(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.answer(
        "Отправьте: Роль флуда,роль стаффа.\n"
        "Пример: Шедлетский,Модератор\n"
        "Для отмены напишите 'отмена'."
    )
    await state.set_state(AdminAddPlace.waiting_staff)


@dp.message(AdminAddPlace.waiting_staff)
async def process_add_place_staff(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    parts = [p.strip() for p in message.text.split(',')]
    if len(parts) < 2:
        await message.answer("❌ Неверный формат. Нужно: Роль флуда,роль стаффа")
        return
    flood_role, staff_role = parts[0], parts[1]
    if not await flood_role_exists(flood_role):
        await message.answer(f"❌ Роль флуда '{flood_role}' не существует.")
        await state.clear()
        return
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT 1 FROM staff_roles WHERE role_name=?", (staff_role,)) as cur:
            if not await cur.fetchone():
                await message.answer(f"❌ Роль стаффа '{staff_role}' не найдена в настройках.")
                await state.clear()
                return
        async with db.execute(
            "SELECT rowid FROM staff_roles WHERE role_name=? AND user_id IS NULL LIMIT 1", (staff_role,)
        ) as cur:
            row = await cur.fetchone()
            if not row:
                await message.answer(f"❌ Нет свободных слотов для роли '{staff_role}'.")
                await state.clear()
                return
            rowid = row[0]
        await db.execute("UPDATE staff_roles SET user_id=NULL WHERE rowid=?", (rowid,))
        await db.commit()
    await log_action(user_id, "Добавление стафф-связки", f"Флуд {flood_role} -> Стафф {staff_role}")
    await update_staff_info()
    await message.answer(f"✅ Связка '{flood_role} -> {staff_role}' добавлена.")
    await state.clear()


# ==================== УДАЛЕНИЕ МЕСТ ====================
@dp.callback_query(F.data == "admin_delete_places")
async def cb_admin_delete_places(callback: CallbackQuery):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await replace_menu(callback, "Выберите, что удалить:", delete_places_menu_keyboard())


@dp.callback_query(F.data == "delete_flood_place")
async def cb_delete_flood_place(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "Введите: Роль,🌸 (например: Шедлетский,🌸)\n"
        "Если место занято — эмодзи будет убран (станет 'Роль:').\n"
        "Если свободно — удалится полностью.\n"
        "Для отмены напишите 'отмена'."
    )
    await state.set_state(AdminDeletePlace.waiting_flood)


@dp.message(AdminDeletePlace.waiting_flood)
async def process_delete_flood_place(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    if len(parts) < 2:
        ...
        return
    role_name, emoji = parts[0], parts[1]
    ok = await remove_flood_place(role_name, emoji)
    if not ok:
        await message.answer(f"❌ Место '{role_name}:{emoji}' не найдено.")
        await state.clear()
        return
    await log_action(user_id, "Удаление места флуда", f"{role_name}:{emoji}")
    await update_flood_info()
    await message.answer(f"✅ Место '{role_name}:{emoji}' обработано.")
    await state.clear()


@dp.callback_query(F.data == "delete_staff_place")
async def cb_delete_staff_place(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "Введите ID пользователя и роль через пробел (например: 123456789 Модератор)\n"
        "Для отмены напишите 'отмена'."
    )
...
async def process_delete_staff_place(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    try:
        ...
        return
    role_name = parts[1]
    await log_action(user_id, "Удаление места стаффа", f"Пользователь {target_id}, роль {role_name}")
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "UPDATE staff_roles SET user_id=NULL, username=NULL WHERE user_id=? AND role_name=?",
            (target_id, role_name)
        ...
        await db.commit()
    await update_staff_info()
    await message.answer("Место стаффа освобождено.")
    await state.clear()


@dp.callback_query(F.data == "delete_rest_place")
async def cb_delete_rest_place(callback: CallbackQuery, state: FSMContext):
...
async def process_delete_rest_place(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
...
async def cb_admin_list_admins(callback: CallbackQuery):
    ...
    text = "👑 **Владельцы:**\n"
    text += "\n".join(owners_list) if owners_list else "Нет владельцев"
    text += "\n\n🛡 **Админы:**\n"
    text += "\n".join(admins_list) if admins_list else "Нет админов"
    await callback.message.edit_text(text, parse_mode="Markdown")
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    if any([
        ...
        await bot.send_message(target_chat_id, text, entities=entities)
        if target == "info":
            await save_info_message_entities("flood_roles", entities)
...
# ==================== Отправить шаблон ====================
@dp.callback_query(F.data == "admin_send_template")
async def cb_admin_send_template(callback: CallbackQuery):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await replace_menu(callback, "Выберите категорию шаблона:", send_template_category_keyboard())


@dp.callback_query(F.data.startswith("send_tmpl_"))
async def cb_send_template_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[2]
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT value FROM settings WHERE key=?", (f"{category}_template",)) as cur:
            ...
            template = row[0] if row else None
    if not template:
        await callback.answer(f"Шаблон для '{category}' не установлен.", show_alert=True)
        return
    await state.update_data(template_category=category, template_text=template)
    await callback.message.answer(
        "Отправьте дополнительный текст, который будет добавлен к шаблону (или напишите '-' если не нужно).\n"
        "Для отмены напишите 'отмена'."
    )
    await state.set_state(AdminSendTemplate.waiting_text)


@dp.message(AdminSendTemplate.waiting_text)
async def process_send_template(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    if category == "flood":
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT role_name, emoji FROM flood_roles") as cur:
                roles = await cur.fetchall()
        roles_list = "\n".join([f"{role}:{emoji}" for role, emoji in roles]) if roles else "Нет ролей"
    elif category == "staff":
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT value FROM settings WHERE key='staff_settings'") as cur:
                settings_row = await cur.fetchone()
                if settings_row:
                    ...
                    roles_list = "Нет ролей"
    elif category == "rest":
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT role_name, expiry_date FROM rest_roles") as cur:
                roles = await cur.fetchall()
        roles_list = "\n".join([f"{role}: {expiry}" for role, expiry in roles]) if roles else "Нет ролей"
    else:
        roles_list = ""
    final_text = template.replace("{roles}", roles_list)
    if message.text and message.text.strip() != "-":
        final_text += "\n\n" + message.text
    # Отправляем в правильный канал в зависимости от категории
    if category == "rest":
        await bot.send_message(INFO_CHANNEL_ID, final_text)
        await log_action(user_id, f"Отправка шаблона в info ({category})", "")
    else:
        await bot.send_message(GROUP_FLOOD_ID, final_text)
        await log_action(user_id, f"Отправка шаблона в {category}", "")
...
async def cb_allow_app_category(callback: CallbackQuery, state: FSMContext):
    ...
    if not await is_admin(user_id):
        ...
        return
    await callback.message.answer(
        f"Введите user_id пользователя, которому разрешить подавать заявки в категорию '{category}'.\n"
        "Для отмены напишите 'отмена'."
    )
...
async def cb_template_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "Для стаффа доступны **{roles}** (счётчики) и **{people}** (список людей).\n"
        "Пример:\n"
        "📋 Список ролей во флуде:\n{roles}\n\n"
        "Для отмены напишите 'отмена'.\n"
        "Чтобы удалить шаблон, напишите 'обнулить'."
    )
...
async def process_template(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        await state.clear()
        return
    if message.text and message.text.lower() == "обнулить":
        data = await state.get_data()
        category = data.get("category")
        if category:
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("DELETE FROM settings WHERE key=?", (f"{category}_template",))
                await db.commit()
            await log_action(user_id, "Удаление шаблона", f"Категория {category}")
            await message.answer(f"✅ Шаблон для '{category}' удалён.")
            if category == "flood":
                await update_flood_info()
            elif category == "staff":
                await update_staff_info()
...
async def process_bind_link(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Сообщение с ID {msg_id} привязано к категории '{category}'. "
        "Теперь бот будет обновлять его автоматически."
    )
    await state.clear()
...
async def process_edit_info_new_text(message: Message, state: FSMContext):
    ...
    try:
        if any([
            ...
            async with aiosqlite.connect("bot.db") as db:
                async with db.execute(
                    "SELECT key FROM info_messages WHERE message_id=?", (str(old_msg_id),)
                ) as cur:
            ...
            try:
                ...
                logger.warning(f"Не удалось отредактировать {old_msg_id}: {e}. Пересоздаю.")
                await bot.delete_message(INFO_CHANNEL_ID, old_msg_id)
                new_msg = await bot.send_message(INFO_CHANNEL_ID, new_text, entities=entities)
                async with aiosqlite.connect("bot.db") as db:
                    async with db.execute(
                        "SELECT key FROM info_messages WHERE message_id=?", (str(old_msg_id),)
                    ) as cur:
                    ...
                    for key_row in keys:
                        ...
                        await db.execute("UPDATE info_messages SET message_id=? WHERE key=?", (str(new_msg.message_id), key))
                    await db.commit()
                await log_action(user_id, "Редактирование инфо (текст, пересоздание)", f"{old_msg_id} -> {new_msg.message_id}")
                await message.answer(f"✅ Сообщение заменено!\n\nНовый текст:\n{new_text[:200]}")
        await state.clear()
        status = await get_user_status(user_id)
        is_member = await is_user_in_flood_group(user_id)
        await message.answer("Выберите раздел:", reply_markup=main_menu_keyboard(status, is_member))
...
async def handle_fuck_1x4(message: Message):
    try:
        ...
        target_mention = f"@{target_user.username}" if target_user.username else f"[{target_user.full_name}](tg://user?id={target_user.id})"
        sender_mention = f"@{message.from_user.username}" if message.from_user.username else f"[{message.from_user.full_name}](tg://user?id={message.from_user.id})"
        ...
        if message.reply_to_message:
            if "{mention}" in reply_template:
                final_reply = reply_template.replace("{mention}", target_mention)
            else:
                final_reply = f"{sender_mention} {reply_template} {target_mention}"
        else:
            if "{mention}" in reply_template:
                final_reply = reply_template.replace("{mention}", sender_mention)
            else:
                final_reply = f"{sender_mention} {reply_template}"
...
async def new_member(message: Message):
    ...
    for new_user in message.new_chat_members:
        ...
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute(
                "SELECT user_id FROM users WHERE in_group=1 AND user_id != ?", (user_id,)
            ) as cur:
        ...
        while idx < len(other_members):
            chunk_size = random.randint(5, 7)
            chunk = other_members[idx:idx + chunk_size]
            idx += chunk_size
            if not chunk:
                continue
            random.shuffle(emojis)
            # Скрытые упоминания через эмодзи-ссылки (без текстовых ссылок)
            hidden_mentions = ''.join(
                f'<a href="tg://user?id={uid}">{emojis[i % len(emojis)]}</a>'
                for i, uid in enumerate(chunk)
            )
            if first:
                # Первое сообщение: "Нью Роль" + скрытые упоминания
                await message.answer(f"{base_text}\n\n{hidden_mentions}", parse_mode="HTML")
                first = False
            else:
                # Последующие: только скрытые упоминания
                await message.answer(hidden_mentions, parse_mode="HTML")
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await replace_menu(callback, "История очищена.", await admin_panel_keyboard(user_id))
...
async def on_startup():
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('ghost_mode', '0')")
        await db.commit()
    await cleanup_expired_bans()
    await cleanup_expired_app_bans()
    try:
...
async def main():
    await on_startup()
    await asyncio.gather(dp.start_polling(bot), start_web())
