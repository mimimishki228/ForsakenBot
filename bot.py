...
async def process_edit_info_link(message: Message, state: FSMContext):
    ...
    try:
        msg = await bot.get_messages(INFO_CHANNEL_ID, msg_id)
        if not msg:
            await message.answer("❌ Сообщение не найдено. Проверьте ID и права бота.")
            return
        
        current_text = msg.text or "Медиа-сообщение"
        ...
        await message.answer(f"❌ Ошибка при получении сообщения: {e}")
