...
LOG_GROUP_LINK = os.getenv("LOG_GROUP_LINK", "https://t.me/+XXXXXXXXXXX")
...
async def log_action(user_id: int, action: str, details: str = ""):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    try:
        text = f"[{timestamp[:19]}] Пользователь {user_id}: {action} {details}"
        await bot.send_message(LOG_GROUP_ID, text)
    except Exception as e:
        logging.error(f"Не удалось отправить лог в группу: {e}")
...
# ---------- Клавиатуры для подтверждения заявок ----------
...
async def cb_admin_logs(callback: CallbackQuery):
    ...
    if not rows:
        ...
        return
    lines = []
    for user_id, action, details, ts in rows:
        lines.append(f"[{ts[:19]}] Пользователь {user_id}: {action} {details}")
...
async def app_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✅ Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
...
async def app_edit(callback: CallbackQuery, state: FSMContext):
    ...
    if edit_count >= 7:
        ...
        if app_text:
            ...
            await callback.message.edit_text("✅ Вы использовали все 7 попыток редактирования. Заявка автоматически отправлена.")
            await state.clear()
            return
        else:
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "Для отмены напишите 'отмена'.\n"
        "Чтобы закрыть чат, напишите 'удалить чат поддержки'."
    )
...
async def support_close_command(message: Message):
    ...
    if not message.text:
        return
    if message.text.lower() == "удалить чат поддержки":
        async with aiosqlite.connect("bot.db") as db:
            await db.execute("UPDATE support_tickets SET status='closed' WHERE user_id=? AND status='active'", (user_id,))
            await db.commit()
        await message.answer("✅ Тикет закрыт. Вы можете снова написать в поддержку.")
        await log_action(user_id, "Закрыл тикет", "")
...
async def cb_admin_tickets(callback: CallbackQuery):
    ...
    if not tickets:
        ...
        return
    kb = []
    for ticket_id, user_id, created_at in tickets:
        try:
            ...
            label = f"Тикет #{ticket_id} - ID:{user_id}"
...
# ---------- Обработка ответов админа на заявки (универсальный, без привязки к reply) ----------
@dp.message(F.text)
async def admin_application_reply(message: Message):
    if not await is_admin(message.from_user.id):
        return
    if message.chat.id == GROUP_FLOOD_ID:
        return  # не обрабатываем в группе

    text = message.text.strip().lower()
    if not text.startswith(("принять", "отказать", "отказано")):
        return

    # Определяем, есть ли ID в тексте или берём из reply
    applicant_id = None
    original_msg = message.reply_to_message
    if original_msg and original_msg.text:
        # Пытаемся извлечь ID из reply
        match = re.search(r"User ID:\s*(\d+)", original_msg.text)
        if match:
            applicant_id = int(match.group(1))
    if not applicant_id:
        # Пытаемся найти ID в тексте команды (первое число после команды)
        # Например "принять 123456789,роль,эмодзи" или "принять 123456789 роль эмодзи"
        # Ищем число в тексте после команды
        parts = re.split(r'\s+|,', text)
        for part in parts:
            if part.isdigit():
                applicant_id = int(part)
                break
    if not applicant_id:
        await message.answer("❌ Не удалось определить ID пользователя. Укажите ID в сообщении или ответьте на сообщение заявки.")
        return

    # Определяем категорию заявки по reply или по наличию ключевых слов
    if original_msg and original_msg.text:
        if "Заявка во флуд" in original_msg.text:
            ...
            await message.answer("❌ Не удалось определить тип заявки.")
            return
    else:
        # Если нет reply, пытаемся определить по тексту команды (например, "принять флуд")
        # Но проще запросить у админа явно
        await message.answer("❌ Ответьте на сообщение заявки или укажите тип заявки в тексте.")
        return

    if applicant_id == message.from_user.id:
    ...
    # Убираем команду и ID, остальное — параметры
    # Удаляем из текста команду и ID
    cleaned = text
    # Удаляем команду
    for cmd in ["принять", "отказать", "отказано"]:
        if cleaned.startswith(cmd):
            cleaned = cleaned[len(cmd):].strip()
            break
    # Удаляем ID, если он был в тексте
    if applicant_id:
        cleaned = re.sub(r'\b' + str(applicant_id) + r'\b', '', cleaned).strip()
    # Убираем лишние запятые и пробелы
    cleaned = re.sub(r'[, ]+', ',', cleaned).strip(',')
    parts = [p.strip() for p in cleaned.split(',') if p.strip()]

    comment = "Без комментария"
    # Если есть комментарий (часть после параметров)
    if len(parts) > 0 and not parts[-1].startswith(("роль", "эмодзи", "время")):
        # считаем, что последний элемент — комментарий, но лучше оставить как есть
        pass

    command = text.split()[0]  # "принять" или "отказать"

    if command.startswith("принять"):
        if table == "flood_applications":
            if len(parts) < 2:
                await message.answer("❌ Формат: Принять ID,Роль,Эмодзи\nПример: Принять 123456789,Шедлетский,🔫")
                return
            role_name = parts[0]
            emoji = parts[1]
            ...
            if len(parts) < 2:
                await message.answer("❌ Формат: Принять ID,Роль,Время\nПример: Принять 123456789,Рест,7д")
                return
            role_name = parts[0]
            expiry = parts[1]
            ...
            if len(parts) < 2:
                await message.answer("❌ Формат: Принять ID,РольФлуда,РольСтаффа\nПример: Принять 123456789,Шедлетский,Модератор")
                return
            flood_role = parts[0]
            staff_role = parts[1]
        ...
        await log_action(message.from_user.id, f"Принял {action_name}", f"Пользователь {applicant_id}")
        ...
        # Обновляем сообщение заявки, если оно было reply
        if original_msg:
            try:
                await bot.edit_message_text(
                    chat_id=message.chat.id,
                    message_id=original_msg.message_id,
                    text=original_msg.text.replace("🕒 Ожидает", "✅ Принята")
                )
            except Exception as e:
                logging.warning(f"Не удалось обновить сообщение: {e}")

        await message.answer(f"✅ Заявка одобрена. Уведомление отправлено.")
        ...
        await log_action(message.from_user.id, f"Отклонил {action_name}", f"Пользователь {applicant_id}, причина: {comment[:50]}")
        ...
        if original_msg:
            try:
                await bot.edit_message_text(
                    chat_id=message.chat.id,
                    message_id=original_msg.message_id,
                    text=original_msg.text.replace("🕒 Ожидает", "❌ Отклонена")
                )
            except Exception as e:
                logging.warning(f"Не удалось обновить сообщение: {e}")

        await message.answer(f"❌ Заявка отклонена. Уведомление отправлено.")
    else:
...
async def show_applications(chat_id: int, table: str, title: str, format_hint: str):
    ...
    if not apps:
        ...
        for user_id, app_text, status in apps:
            ...
            text = f"📩 {title}\nUser ID: {user_id}\nСтатус: {status_label}\nАнкета: {app_text}\n\nОтветьте на это сообщение командой или напишите в чат:\n{format_hint}"
...
async def cb_admin_app_flood(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "flood_applications", "Заявка во флуд", "Формат: Принять ID,Роль,Эмодзи\nили Отказать ID,Причина")
...
async def cb_admin_app_rest(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "rest_applications", "Заявка в рест", "Формат: Принять ID,Роль,Время\nили Отказать ID,Причина")
...
async def cb_admin_app_staff(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "staff_applications", "Заявка в стафф", "Формат: Принять ID,РольФлуда,РольСтаффа\nили Отказать ID,Причина")
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Назначил админа", f"Пользователь {user_id}")
...
async def process_remove_admin(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Удалил админа", f"Пользователь {user_id}")
...
async def process_ban_app(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Запрет на заявки", f"Пользователь {user_id}, категория {category}")
...
async def process_allow_app(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Разрешил заявки", f"Пользователь {user_id}, категория {category}")
...
async def process_cooldown_give(message: Message, state: FSMContext):
    ...
    await log_action(message.from_user.id, "Выдал кд", f"Пользователь {user_id}, категория {category}, {duration_readable}")
...
async def process_cooldown_remove(message: Message, state: FSMContext):
    ...
    await log_action(message.from_user.id, "Снял кд", f"Пользователь {user_id}, категория {category}")
...
async def process_cooldown_settings(message: Message, state: FSMContext):
    ...
    await log_action(message.from_user.id, "Настройка кд", f"Категория {category}, {duration_readable}")
...
async def process_template(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Настройка шаблона", f"Категория {category}")
...
async def process_bind_link(message: Message, state: FSMContext):
    ...
    await log_action(message.from_user.id, "Привязка info", f"Категория {category}, ID {msg_id}")
...
async def cb_fuck_toggle(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(callback.from_user.id, "Toggle fuck 1x4", f"Новое состояние: {new}")
...
async def process_fuck_trigger(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Настройка триггера 1x4", trigger)
...
async def process_fuck_reply(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(message.from_user.id, "Настройка ответа 1x4", reply)
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(callback.from_user.id, "Очистка истории заявок", f"Категория {category}")
