...
# ---------- Работа с логами ----------
async def add_log(action: str, user_id: int, details: str = ""):
...
async def get_logs(limit: int = 100) -> list:
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT timestamp, user_id, action, details FROM logs ORDER BY timestamp DESC LIMIT ?",
            (limit,)
        ) as cur:
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    ...
    await add_log("assign_flood_role", user_id, f"Роль: {role_name}, эмодзи: {emoji}")
...
async def revoke_flood_role(user_id: int):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_flood_info()
    await add_log("revoke_flood_role", user_id, "Роль отозвана")
...
async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    ...
    await add_log("assign_rest_role", user_id, f"Роль: {role_name}, до: {expiry_date}")
...
async def revoke_rest_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_rest_info()
    await add_log("revoke_rest_role", user_id, f"Роль: {role_name}")
...
async def assign_staff_role(user_id: int, role_name: str, username: str):
    ...
    await add_log("assign_staff_role", user_id, f"Роль: {role_name}, username: {username}")
    return True
...
async def revoke_staff_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_staff_info()
    await add_log("revoke_staff_role", user_id, f"Роль: {role_name}")
...
async def set_cooldown(user_id: int, category: str, seconds: int):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await add_log("set_cooldown", user_id, f"Категория: {category}, длительность: {seconds}с")
...
async def remove_cooldown(user_id: int, category: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await add_log("remove_cooldown", user_id, f"Категория: {category}")
...
async def set_cooldown_duration(category: str, seconds: int):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("INSERT OR REPLACE INTO cooldown_settings (category, duration_seconds) VALUES (?, ?)",
                         (category, seconds))
        await db.commit()
    await add_log("set_cooldown_duration", 0, f"Категория: {category}, длительность: {seconds}с")
...
async def cmd_start(message: Message):
    ...
    await add_log("start_bot", user_id, "Запуск бота")
...
async def cb_admin_logs(callback: CallbackQuery):
    ...
    for timestamp, log_user_id, action, details in logs:
        dt = datetime.fromisoformat(timestamp)
        time_str = dt.strftime("%d.%m %H:%M")
        lines.append(f"`{time_str}` | {action} | User: {log_user_id} | {details}")
    
    text = "\n".join(lines[:20])  # Показываем 20 последних записей
    if len(lines) > 20:
        text += f"\n... и ещё {len(lines)-20} записей"
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("submit_application", user_id, "Категория: flood")
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
    await add_log("submit_application", user_id, "Категория: rest")
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
    await add_log("submit_application", user_id, "Категория: staff")
...
async def process_support_start(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("submit_support", user_id, f"Сообщение: {message.text[:50]}...")
...
async def admin_reply(message: Message):
    ...
    if "Новое обращение в поддержку" in original_msg.text or "Ответ от администратора" in original_msg.text:
        try:
            ...
            await add_log("support_reply", admin_id, f"Пользователю: {user_id}")
    ...
    # Проверяем, не является ли админ заявителем
    ...
    if command.startswith("принять"):
        ...
        await add_log("approve_application", admin_id, f"Категория: {category}, пользователь: {applicant_id}")
        ...
        await add_log("reject_application", admin_id, f"Категория: {category}, пользователь: {applicant_id}")
    else:
...
async def support_reply(message: Message):
    ...
    await add_log("user_support_reply", user_id, f"Сообщение: {message.text[:50]}...")
...
async def cb_my_cooldowns(callback: CallbackQuery):
    ...
    await add_log("view_cooldowns", user_id, "Просмотр своих кд")
...
async def show_applications(chat_id: int, table: str, title: str):
    ...
    if not apps:
        ...
        for user_id, app_text, status in apps:
            ...
            # Разный формат для разных категорий
            if table == "flood_applications":
                format_text = "Принять,Роль,Эмодзи\nили\nОтказать,Причина"
            elif table == "rest_applications":
                format_text = "Принять,Роль,Время\nили\nОтказать,Причина"
            elif table == "staff_applications":
                format_text = "Принять,Роль\nили\nОтказать,Причина"
            else:
                format_text = "Принять,Роль\nили\nОтказать,Причина"
...
async def process_ban(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("ban_user", actor_id, f"Забанен: {target_id}")
...
async def process_unban(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("unban_user", actor_id, f"Разбанен: {target_id}")
...
async def cb_toggle_flood(callback: CallbackQuery):
    ...
    await add_log("toggle_applications", callback.from_user.id, f"Категория: flood, состояние: {'закрыты' if new == '0' else 'открыты'}")
...
async def cb_toggle_rest(callback: CallbackQuery):
    ...
    await add_log("toggle_applications", callback.from_user.id, f"Категория: rest, состояние: {'закрыты' if new == '0' else 'открыты'}")
...
async def cb_toggle_staff(callback: CallbackQuery):
    ...
    await add_log("toggle_applications", callback.from_user.id, f"Категория: staff, состояние: {'закрыты' if new == '0' else 'открыты'}")
...
async def process_flood_settings(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("update_flood_roles", message.from_user.id, f"Обновлены роли")
...
async def process_staff_settings(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("update_staff_roles", message.from_user.id, f"Обновлены роли")
...
async def process_delete_flood_role(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_flood_role", message.from_user.id, f"Роль: {role_name}")
...
async def process_delete_staff_role(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_staff_role", message.from_user.id, f"Пользователь: {user_id}, роль: {role_name}")
...
async def process_delete_rest_role(message: Message, state: FSMContext):
    ...
    await message.answer(f"Роль '{role_name}' удалена.")
    await state.clear()
    await add_log("delete_rest_role", message.from_user.id, f"Роль: {role_name}")
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("add_admin", message.from_user.id, f"Новый админ: {user_id}")
...
async def process_remove_admin(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("remove_admin", message.from_user.id, f"Удален админ: {user_id}")
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("send_message", message.from_user.id, f"В чат: {target_chat_id}")
...
async def process_delete_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_message", message.from_user.id, f"Из чата: {chat_id}, ID: {msg_id}")
...
async def process_ban_app(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("ban_application", message.from_user.id, f"Категория: {category}, пользователь: {user_id}")
...
async def process_allow_app(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("allow_application", message.from_user.id, f"Категория: {category}, пользователь: {user_id}")
...
async def process_template(message: Message, state: FSMContext):
    ...
    if category == "flood":
        ...
        await update_rest_info()
    await state.clear()
    await add_log("update_template", message.from_user.id, f"Категория: {category}")
...
async def process_bind_link(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("bind_message", message.from_user.id, f"Категория: {category}, ID: {msg_id}")
...
async def cb_fuck_toggle(callback: CallbackQuery):
    ...
    await replace_menu(callback, "Настройка функции 'Ебать 1x4':", fuck_menu_keyboard())
    await add_log("toggle_fuck", callback.from_user.id, f"Состояние: {'включена' if new == '1' else 'выключена'}")
...
async def process_fuck_trigger(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("set_fuck_trigger", message.from_user.id, f"Триггер: {trigger}")
...
async def process_fuck_reply(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("set_fuck_reply", message.from_user.id, f"Ответ: {reply}")
...
async def cb_destroy_yes(callback: CallbackQuery):
    ...
    try:
        ...
        await add_log("destroy_group", callback.from_user.id, "Группа уничтожена")
    except Exception as e:
...
async def new_member(message: Message):
    if message.chat.id == GROUP_FLOOD_ID:
        for new_user in message.new_chat_members:
            ...
            await add_log("join_group", user_id, "Присоединился к группе")
...
async def left_member(message: Message):
    if message.chat.id == GROUP_FLOOD_ID:
        ...
        await add_log("left_group", user_id, "Покинул группу")
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await add_log("clear_history", callback.from_user.id, f"Категория: {category}")
