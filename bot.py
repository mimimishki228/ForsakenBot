...
def admin_panel_keyboard(user_id: int):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🍆 Ебать 1x4", callback_data="admin_fuck_menu")],  # доступен всем админам
    ]
...
async def cb_admin_fuck_menu(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):  # теперь доступно всем админам, не только владельцам
        await callback.answer("Недостаточно прав", show_alert=True)
        return
