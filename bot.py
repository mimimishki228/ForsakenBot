...
async def new_member(message: Message):
    if message.chat.id == GROUP_FLOOD_ID:
        for new_user in message.new_chat_members:
            user_id = new_user.id
            # Помечаем нового участника как находящегося в группе
            await mark_user_in_group(user_id)

            # Отправляем приветственное сообщение "Нью" с ролью, если есть
            role = None
            async with aiosqlite.connect("bot.db") as db:
            ...
            # Получаем список всех остальных участников (in_group=1, кроме нового)
            async with aiosqlite.connect("bot.db") as db:
            ...
            # Разбиваем на части по 10 человек и отправляем скрытые упоминания
            chunk_size = 10
