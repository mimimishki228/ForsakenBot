...
async def delete_info_message(key: str):
    ...
    if msg_id:
        try:
            await bot.delete_message(INFO_CHANNEL_ID, msg_id)
        except:
            pass
        async with aiosqlite.connect("bot.db") as db:
...
async def send_or_edit_info_message(key: str, text: str):
    """Отправляет новое сообщение в info или редактирует существующее."""
    msg_id = await get_info_message_id(key)
    if msg_id:
        try:
            await bot.edit_message_text(text, INFO_CHANNEL_ID, msg_id)
        except:
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    ...
    try:
        await bot.send_message(user_id, f"✅ Ваша заявка во флуд одобрена! Ссылка на группу: {FLOOD_GROUP_LINK}")
    except:
        pass
    # Обновляем info
...
async def revoke_flood_role(user_id: int):
    """Убирает пользователя из роли флуда и возвращает статус."""
    async with aiosqlite.connect("bot.db") as db:
...
async def update_flood_info():
    ...
    for role_name, emoji, user_id in roles:
        if user_id is None:
            lines.append(f"{role_name}: {emoji} (свободна)")
        else:
            lines.append(f"{role_name}: {emoji} (занята)")
    text = "\n".join(lines)
    await send_or_edit_info_message("flood_roles", text)
...
async def update_rest_info():
    ...
    for role_name, expiry, user_id in roles:
        if user_id is None:
            lines.append(f"{role_name}: до {expiry} (свободна)")
        else:
            lines.append(f"{role_name}: до {expiry} (занята)")
    text = "\n".join(lines)
    await send_or_edit_info_message("rest_roles", text)
...
async def update_staff_info():
    async with aiosqlite.connect("bot.db") as db:
        ...
        text = "\n".join(lines)
    await send_or_edit_info_message("staff_roles", text)
...
def main_menu_keyboard(user_id: int = None):
    ...
    if user_id:
        # Синхронно получить статус нельзя, поэтому используем заглушку или передаём статус отдельно
        # В обработчиках будем формировать клавиатуру с учётом статуса
        pass
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def user_panel_keyboard(status: str = "none"):
    buttons = []
    if status == "flood":
...
async def cmd_start(message: Message):
    ...
    await message.answer("Добро пожаловать! Выберите раздел:",
                         reply_markup=main_menu_keyboard_with_status(status))

def main_menu_keyboard_with_status(status: str):
    buttons = [
        [InlineKeyboardButton(text="👤 Панель участника", callback_data="panel_user")],
        [InlineKeyboardButton(text="🛠 Панель админа", callback_data="panel_admin")],
        [InlineKeyboardButton(text="ℹ️ Info", url=INFO_CHANNEL_LINK)]
...
async def cb_main_menu(callback: CallbackQuery):
    ...
    status = await get_user_status(user_id)
    await callback.message.edit_text("Главное меню:", reply_markup=main_menu_keyboard_with_status(status))
...
async def cb_panel_user(callback: CallbackQuery):
    ...
    if await is_banned(user_id):
        ...
        return
    status = await get_user_status(user_id)
    await callback.message.edit_text("Панель участника:", reply_markup=user_panel_keyboard(status))
...
# ---------- Админ: просмотр заявок (каждая отдельным сообщением) ----------
async def show_applications(chat_id: int, table: str, title: str):
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(f"SELECT user_id, text FROM {table} WHERE status='pending'") as cur:
            apps = await cur.fetchall()
    if not apps:
        await bot.send_message(chat_id, f"Нет активных заявок: {title}")
        return
...
async def toggle_app_setting(setting_key: str, info_key: str, text_on: str, text_off: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    # Удаляем предыдущее сообщение о статусе
...
# ---------- Админ: отправка в info ----------
@dp.callback_query(F.data == "admin_send_info")
async def cb_admin_send_info(callback: CallbackQuery, state: FSMContext):
    if not await is_owner(callback.from_user.id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    await callback.message.answer("Введите текст сообщения для отправки в info-канал:")
    await state.set_state(AdminSendInfo.waiting_text)

@dp.message(AdminSendInfo.waiting_text)
async def process_send_info(message: Message, state: FSMContext):
    text = message.text
    msg = await bot.send_message(INFO_CHANNEL_ID, text)
    # Сохраняем как произвольное сообщение (не для ролей)
    await set_info_message_id("custom_info", msg.message_id)
    await message.answer("Сообщение отправлено.")
    await state.clear()

# ---------- Админ: уничтожить (заглушка) ----------
...
async def admin_reply(message: Message):
    ...
    if "Заявка во флуд" in original_msg.text:
        if text.startswith("Принять"):
            ...
            emoji = parts[2].strip()
            # Определяем user_id из текста заявки (можно было бы хранить отдельно, но попробуем вытащить)
            # В тексте заявки есть строка "👤 User ID: {user_id}", извлечём
            try:
            ...
            try:
                ...
                await message.answer("Не удалось определить ID заявителя.")
                return
            # Если пользователь уже был принят (статус flood), снимаем роль
            if await get_user_status(applicant_id) == "flood":
    ...
    # Аналогично для реста и стаффа можно добавить (но для краткости оставлено)
    # ...

    await message.answer("Неизвестный тип заявки.")
...
async def on_startup():
    async with aiosqlite.connect("bot.db") as db:
        # Создание таблиц (включая info_messages)
        await db.execute("""
