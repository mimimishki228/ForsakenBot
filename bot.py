...
async def admin_application_reply(message: Message):
    ...
    logging.info(f"Admin reply received from {message.from_user.id}: {message.text}")
    ...
    try:
        ...
        text = message.text.strip()  # убираем ведущие и завершающие пробелы
        ...
        if cmd_lower.startswith("принять"):
            ...
            # Обновляем сообщение заявки – заменяем любой из возможных вариантов статуса
            ...
            # Если заявка уже была принята, отзываем роли
            if current_status == "approved":
                if category == "flood":
                    await revoke_flood_role(applicant_id)
                elif category == "rest":
        ...
        logging.error(f"Ошибка в admin_application_reply: {e}", exc_info=True)
        try:
            await message.answer(f"❌ Произошла внутренняя ошибка: {e}")
        except:
            pass

# ---------- ИСПРАВЛЕННЫЙ обработчик ответов пользователя в активном тикете ----------
# Теперь он НЕ перехватывает сообщения из группы флуда и НЕ перехватывает reply-сообщения
@dp.message(StateFilter(None), F.text, F.chat.id != GROUP_FLOOD_ID, F.reply_to_message.is_(None))
async def support_reply(message: Message):
    ...
    # Если пользователь ввёл команду закрытия – пропускаем (она обрабатывается в support_close_command)
...
async def handle_fuck_1x4(message: Message):
    # ---- Логирование ----
    logging.info(f"Fuck trigger check in group: {message.text}")
    ...
    try:
        ...
        logging.error(f"Ошибка в handle_fuck_1x4: {e}", exc_info=True)
