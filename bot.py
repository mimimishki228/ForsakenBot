...
async def delete_application(user_id: int, category: str):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        # Сбрасываем статус только если он соответствует этой категории
        status_map = {
            "flood": "pending_flood",
            "rest": "pending_rest",
            "staff": "pending_staff"
        ...
        if current_status == status_map.get(category):
...
def application_sent_keyboard(category: str):
    """Клавиатура для сообщения об успешной отправке заявки"""
    return InlineKeyboardMarkup(inline_keyboard=[
...
def delete_confirm_keyboard(category: str):
    """Клавиатура подтверждения удаления заявки"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"app_delete_confirm_{category}"),
         InlineKeyboardButton(text="❌ Нет, оставить", callback_data="app_delete_cancel")]
    ])

def has_pending_keyboard(category: str):
    """Клавиатура, когда у пользователя уже есть заявка"""
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def app_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    # Показываем сообщение с кнопкой удаления заявки
    await callback.message.edit_text(
...
async def app_delete(callback: CallbackQuery):
    ...
    # Проверяем, действительно ли у пользователя есть заявка
    status = await get_user_status(user_id)
    expected_status = f"pending_{category}"
...
@dp.callback_query(F.data.startswith("app_delete_confirm_"))
async def app_delete_confirm(callback: CallbackQuery):
    ...
    await log_action(user_id, f"Удалил заявку в {category}", "")
...
@dp.callback_query(F.data == "app_delete_cancel")
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        # Удаляем только принятые и отклонённые заявки
        await db.execute(f"DELETE FROM {table} WHERE status IN ('approved', 'rejected')")
        # Сбрасываем статус только у пользователей, у которых были удалены заявки
        await db.execute("UPDATE users SET status='none' WHERE status IN ('approved', 'rejected')")
        await db.commit()
    
    await log_action(callback.from_user.id, "Очистка истории заявок", f"Категория {category} (только обработанные)")
    await callback.answer(f"Обработанные заявки ({category}) очищены. Ожидающие сохранены.", show_alert=True)
    await replace_menu(callback, "История очищена (обработанные заявки удалены, ожидающие остались).", await admin_panel_keyboard(callback.from_user.id))
...
async def on_startup():
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    # Отправляем тестовое сообщение в группу логов
    try:
