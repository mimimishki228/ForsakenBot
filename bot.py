...
async def send_or_edit_info_message(key: str, text: str):
    """Сначала пытается РЕДАКТИРОВАТЬ сообщение, если не получается — удаляет и создаёт новое."""
    msg_id = await get_info_message_id(key)
    if msg_id:
        try:
            ...
            logger.warning(f"Не удалось отредактировать info-сообщение {key} (id={msg_id}): {e}. Удаляю и создаю новое.")
            try:
                await bot.delete_message(INFO_CHANNEL_ID, int(msg_id))
            except Exception as de:
                logger.warning(f"Не удалось удалить старое info-сообщение {key}: {de}")
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("DELETE FROM info_messages WHERE key=?", (key,))
                await db.commit()
...
async def check_flood_role_available(role_name: str) -> bool:
    """Проверяет, что роль существует И свободна."""
    async with aiosqlite.connect("bot.db") as db:
...
async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    # Нормализуем expiry_date: убираем "ДО ", "до " и пробелы
    expiry_clean = expiry_date.strip()
    expiry_clean = re.sub(r'^(ДО|до)\s*', '', expiry_clean, flags=re.IGNORECASE).strip()
    
    # Если это число + д, оставляем как есть, иначе пробуем распарсить
    if not re.match(r'^\d+\s*[дчмс]?$', expiry_clean, re.IGNORECASE):
        expiry_clean = expiry_clean.lower()
...
async def update_rest_info():
    ...
    if not roles:
        ...
        return
    roles_list = "\n".join([f"{role}: {expiry}" for role, expiry, user_id in roles])
    async with aiosqlite.connect("bot.db") as db:
    ...
    if template:
        ...
        lines = ["🍽 Ресты:"]
        lines.extend([f"{role}: {expiry}" for role, expiry, user_id in roles])
        final_text = "\n".join(lines)
...
async def process_send_template(message: Message, state: FSMContext):
    ...
    # Получаем актуальный список ролей
    if category == "flood":
        async with aiosqlite.connect("bot.db") as db:
                ...
                if settings_row:
                    ...
                    for role_line in roles:
                        ...
                        if len(parts) == 2:
                            role_name = parts[0].strip()
                            limit = int(parts[1].strip())
                            async with db.execute("SELECT COUNT(*) FROM staff_roles WHERE role_name=?", (role_name,)) as c:
    ...
    try:
        if any([
            ...
            try:
                ...
                logger.warning(f"Не удалось отредактировать сообщение {old_msg_id}: {e}. Удаляю и создаю новое.")
                await bot.delete_message(INFO_CHANNEL_ID, old_msg_id)
                ...
                async with aiosqlite.connect("bot.db") as db:
                    ...
                    for key_row in keys:
                        ...
                        await db.execute("UPDATE info_messages SET message_id=? WHERE key=?", (str(new_msg.message_id), key))
                    await db.commit()
                await log_action(user_id, "Редактирование инфо (текст, пересоздание)", f"Старый ID {old_msg_id} -> Новый ID {new_msg.message_id}")
                await message.answer(f"✅ Сообщение заменено (старое удалено, создано новое)!\n\nНовый текст:\n{new_text[:200]}")
...
# ---------- Обработчик сообщений в группе флуда: "выебать 1x4" (обычный и ответ на сообщение) ----------
...
async def handle_fuck_1x4(message: Message):
    ...
    try:
        ...
        # Определяем цель: автор сообщения в ответе, либо сам отправитель
        ...
        # Формируем ответ по шаблону: "юз" ударил "юз"
...
async def new_member(message: Message):
    if message.chat.id == GROUP_FLOOD_ID:
        for new_user in message.new_chat_members:
            ...
            # Отправляем несколько сообщений с тайными упоминаниями (5-7 эмодзи в каждом)
            if not other_members:
                await message.answer(base_text)
                continue

            # Разбиваем участников на группы по 5-7 для каждого сообщения
            ...
            while idx < len(other_members):
                ...
                if not chunk:
                    continue
                # Тайные упоминания: HTML-ссылки с эмодзи
                emojis = ["🍇", "💶", "👍", "🤫", "✝️", "🐷", "🌟", "🔥", "⚡", "💎", "🎯", "🚀", "🎲", "♠️", "🎪"]
                random.shuffle(emojis)
                hidden_mentions = ''.join(
                    f'<a href="tg://user?id={uid}">{emojis[i % len(emojis)]}</a>' for i, uid in enumerate(chunk)
                )
                if first:
