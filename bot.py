...
async def admin_application_reply(message: Message):
    ...
    # Извлекаем ID пользователя из сообщения заявки
    ...
    if text.lower().startswith(("принять", "отказать", "отказано")):
        # Определяем команду
        if text.lower().startswith("принять"):
            command = "принять"
            # Убираем "принять" из начала
        ...
        elif text.lower().startswith(("отказать", "отказано")):
            command = "отказать"
            ...
            # Отзываем роли, если заявка уже была принята
            if current_status == "approved":
                if category == "flood":
                    await revoke_flood_role(applicant_id)
                elif category == "rest":
