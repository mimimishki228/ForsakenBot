...
class AdminBindInfo(StatesGroup):
    waiting_category = State()
    waiting_link = State()
# Новые состояния для разрешения заявок и настройки "выебать 1x4"
...
def admin_panel_keyboard(user_id: int):
    buttons = [
        ...
        [InlineKeyboardButton(text="✅ Разрешить заявки", callback_data="admin_allow_apps")],  # новая кнопка
    ...
    if is_owner(user_id):
        ...
        # Изменённая кнопка "выебать 1x4" – теперь открывает меню
        buttons.append([InlineKeyboardButton(text="🍆 Ебать 1x4", callback_data="admin_fuck_menu")])
...
# Новая клавиатура для разрешения заявок
...
# Новые клавиатуры для "выебать 1x4"
...
# ---------- Админ: разрешение заявок (новая функция) ----------
...
# ---------- Обработчик сообщений в группе флуда: "выебать 1x4" (обновлённый) ----------
...
async def handle_fuck_1x4(message: Message):
    ...
    # Получаем настройки триггера и ответа
    async with aiosqlite.connect("bot.db") as db:
    ...
    # Проверяем, содержит ли сообщение один из триггеров
