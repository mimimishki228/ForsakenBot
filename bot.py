...
async def is_banned(user_id: int) -> bool:
    """Проверяет, забанен ли пользователь (с учётом срока бана)"""
    async with aiosqlite.connect("bot.db") as db:
...
async def is_application_banned(user_id: int, category: str) -> bool:
    """Проверяет, есть ли активный запрет на заявки (с учётом срока)"""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
...
async def get_app_ban_info(user_id: int, category: str) -> dict:
    """Получает информацию о запрете на заявки"""
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
...
async def cleanup_expired_bans():
    """Удаляет истекшие баны из базы данных"""
    async with aiosqlite.connect("bot.db") as db:
        ...
        if count > 0:
            logger.info(f"Очистка истекших банов: удалено {count} записей")
        return count

async def cleanup_expired_app_bans():
    """Удаляет истекшие запреты на заявки"""
    async with aiosqlite.connect("bot.db") as db:
        deleted = await db.execute(
        ...
        count = deleted.rowcount
        if count > 0:
            logger.info(f"Очистка истекших запретов: удалено {count} записей")
        return count
...
def format_remaining_time(until_date_str: str) -> str:
    """Форматирует оставшееся время до указанной даты в читаемый вид"""
    ...
    # Если время истекло, возвращаем "истек"
    if until <= now:
        return "истек"
    ...
    if seconds > 0 and days == 0:  # Показываем секунды только если нет дней
...
async def cmd_start(message: Message):
    ...
    # Если команда отправлена в группе флуда
...
async def cb_admin_active_bans(callback: CallbackQuery):
    ...
    # Принудительно удаляем все истекшие баны
    deleted_count = await cleanup_expired_bans()
    
    # Получаем активные баны
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id, until_date, reason FROM banned_users WHERE until_date IS NULL OR until_date > datetime('now') ORDER BY user_id"
        ) as cur:
            banned = await cur.fetchall()
    
    if not banned:
        message = "📋 Активных банов нет."
        if deleted_count > 0:
            message += f"\n\n🗑 Удалено истекших банов: {deleted_count}"
...
async def cb_admin_active_app_bans(callback: CallbackQuery):
    ...
    # Принудительно удаляем все истекшие запреты
    deleted_count = await cleanup_expired_app_bans()
    
    # Получаем активные запреты
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT user_id, category, until_date FROM application_bans WHERE until_date IS NULL OR until_date > datetime('now') ORDER BY user_id"
        ) as cur:
            bans = await cur.fetchall()
    
    if not bans:
        message = "🚫 Активных запретов на заявки нет."
        if deleted_count > 0:
            message += f"\n\n🗑 Удалено истекших запретов: {deleted_count}"
        await callback.message.edit_text(message)
        return
...
async def on_startup():
    ...
    # Очищаем истекшие баны и запреты при запуске
