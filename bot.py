...
async def assign_staff_role(user_id: int, role_name: str, username: str):
    async with aiosqlite.connect("bot.db") as db:
        # Используем двойные кавычки для экранирования зарезервированного слова "limit"
...
# ---------- ИСПРАВЛЕННАЯ ФУНКЦИЯ (замена limit на value) ----------
async def update_staff_info():
    async with aiosqlite.connect("bot.db") as db:
        # В таблице settings нет столбца "limit", используем "value"
...
async def on_startup():
    async with aiosqlite.connect("bot.db") as db:
        ...
        # Исправлено: имя столбца limit экранировано двойными кавычками
        await db.execute("""
