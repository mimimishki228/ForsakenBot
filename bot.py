...
# ---- Новые состояния для "Добавить роль" ----
class AdminAddRole(StatesGroup):
...
async def send_or_edit_info_message(key: str, text: str, parse_mode=None, entities=None):
    """Редактирует сообщение, при неудаче — удаляет и создаёт новое.
    Поддерживает parse_mode/entities для сохранения форматирования."""
    msg_id = await get_info_message_id(key)
    if msg_id:
        try:
            if entities:
...
async def check_flood_role_available(role_name: str) -> bool:
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT user_id FROM flood_roles WHERE role_name=?", (role_name,)) as cur:
            row = await cur.fetchone()
            if not row:
                return False
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "UPDATE flood_roles SET user_id=?, emoji=? WHERE role_name=? AND user_id IS NULL",
...
async def revoke_flood_role(user_id: int):
    """Снимает с пользователя роль флуда, НЕ удаляя саму роль, только обнуляет user_id и emoji.
    Т.е. эмодзи рядом с ролью убирается."""
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "UPDATE flood_roles SET user_id=NULL, emoji=NULL WHERE user_id=?",
            (user_id,)
        )
        await db.execute("UPDATE users SET status='none' WHERE user_id=?", (user_id,))
        await db.commit()
...
async def update_flood_info():
    ...
    # Скрытая ссылка на пользователя в эмодзи
    role_lines = []
    for role, emoji, uid in roles:
        if uid is not None and emoji:
            # Скрытая ссылка: <a href="tg://user?id=UID">emoji</a>
            hidden = f'<a href="tg://user?id={uid}">{html.escape(emoji)}</a>'
            role_lines.append(f"{html.escape(role)}:{hidden}")
    ...
    # Пытаемся сохранить форматирование из исходного сообщения, если оно есть
...
async def update_rest_info():
    ...
    for role, expiry, uid in roles:
        # Скрытая ссылка на пользователя в названии роли
        hidden_role = f'<a href="tg://user?id={uid}">{html.escape(role)}</a>'
        role_lines.append(f"{hidden_role}: {html.escape(expiry)}")
    roles_list = "\n".join(role_lines)
...
async def assign_staff_role(user_id: int, role_name: str, username: str = None):
    """username больше не используется для отображения, но оставлен для совместимости."""
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.execute(
            ...
            (user_id, username or str(user_id), rowid)
        )
...
async def update_staff_info():
    async with aiosqlite.connect("bot.db") as db:
        ...
        # Второй блок: список людей по ролям, со скрытой ссылкой
        people_lines = []
        async with db.execute(
            "SELECT role_name, user_id FROM staff_roles WHERE user_id IS NOT NULL ORDER BY role_name"
        ) as cur:
            occupied = await cur.fetchall()
        for role_name, uid in occupied:
            # Имя получаем через get_chat, но чтобы не спамить — просто скрытая ссылка
            try:
                user = await bot.get_chat(uid)
                display = user.full_name or str(uid)
            except Exception:
                display = str(uid)
            hidden_user = f'<a href="tg://user?id={uid}">{html.escape(display)}</a>'
            people_lines.append(f"{hidden_user} - {html.escape(role_name)}")

    top_block = "\n".join(role_lines) if role_lines else "Нет ролей"
    bottom_block = "\n".join(people_lines) if people_lines else ""

    full_roles_list = top_block
    if bottom_block:
        full_roles_list += "\n\n" + bottom_block
    ...
    if template:
        final_text = template.replace("{roles}", full_roles_list)
    else:
        final_text = "🛡 Стафф:\n" + full_roles_list
...
# ==================== Хранение entities для info-сообщений ====================
async def save_info_message_entities(key: str, entities):
    """Сохраняем entities последнего сообщения от админа, чтобы применять форматирование."""
    import json
    if not entities:
        return
...
async def get_info_message_entities(key: str):
    """Возвращает список MessageEntity для сохранения форматирования."""
    import json
...
async def admin_panel_keyboard(user_id: int):
    buttons = [
        ...
        [InlineKeyboardButton(text="➕ Добавить роль", callback_data="admin_add_role")],
...
def add_role_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌊 Во флуд", callback_data="add_role_flood")],
        [InlineKeyboardButton(text="🛡 В стафф", callback_data="add_role_staff")],
        [InlineKeyboardButton(text="🍽 В рест", callback_data="add_role_rest")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="panel_admin")]
    ])
        ...
        if cmd_lower.startswith("принять"):
            ...
            if category == "flood":
                ...
                if await check_flood_role_available(role_name):
                    ...
                    await message.answer(f"❌ Роль '{role_name}' недоступна или занята. Проверьте список ролей.")
                    return
...
# ==================== ДОБАВИТЬ РОЛЬ ====================
@dp.callback_query(F.data == "admin_add_role")
async def cb_admin_add_role(callback: CallbackQuery):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await replace_menu(callback, "Куда добавить роль?", add_role_menu_keyboard())


@dp.callback_query(F.data == "add_role_flood")
async def cb_add_role_flood(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    ...
    await state.set_state(AdminAddRole.waiting_flood)


@dp.message(AdminAddRole.waiting_flood)
async def process_add_role_flood(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.execute("INSERT INTO flood_roles (role_name, emoji) VALUES (?, ?)", (role_name, emoji))
        await db.commit()
    await log_action(user_id, "Добавление роли флуда", f"{role_name}:{emoji}")
    await update_flood_info()
    await message.answer(f"✅ Роль '{role_name}' с эмодзи {emoji} добавлена во флуд.")
    await state.clear()


@dp.callback_query(F.data == "add_role_rest")
async def cb_add_role_rest(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    ...
    await state.set_state(AdminAddRole.waiting_rest)


@dp.message(AdminAddRole.waiting_rest)
async def process_add_role_rest(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    # Создаём заготовку реста (пользователь будет привязан при принятии заявки)
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
...
@dp.callback_query(F.data == "add_role_staff")
async def cb_add_role_staff(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
    ...
    await state.set_state(AdminAddRole.waiting_staff)


@dp.message(AdminAddRole.waiting_staff)
async def process_add_role_staff(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await message.answer("Действие отменено.")
        await state.clear()
    ...
    if not await check_flood_role_available(flood_role):
        await message.answer(f"❌ Роль флуда '{flood_role}' не существует или занята.")
        await state.clear()
        return
    # Назначаем роль стаффа без user_id (свободный слот)
    async with aiosqlite.connect("bot.db") as db:
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    if any([
        ...
        if target == "info":
            await save_info_message_entities("flood_roles", entities)  # для сохранения форматирования
...
async def process_send_template(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Шаблон отправлен в группу флуда.")
    await state.clear()
