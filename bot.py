...
async def cb_panel_user(callback: CallbackQuery):
    ...
    status = await get_user_status(user_id)
    is_member = await is_user_in_flood_group(user_id)
    # Проверяем наличие активного тикета, чтобы скрыть/показать кнопку
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT ticket_id FROM support_tickets WHERE user_id=? AND status='active'", (user_id,)) as cur:
            has_ticket = await cur.fetchone() is not None
    kb = user_panel_keyboard(status, is_member)
    # Можно динамически изменить клавиатуру, но проще оставить всегда, но обработчик будет проверять наличие тикета
    await replace_menu(callback, "Панель участника:", kb)
...
async def app_edit(callback: CallbackQuery, state: FSMContext):
    ...
    # Устанавливаем правильное состояние в зависимости от категории
    if category == "flood":
