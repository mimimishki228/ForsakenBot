...
async def cleanup_expired_bans():
    ...
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("DELETE FROM banned_users WHERE until_date IS NOT NULL AND until_date < datetime('now')")
        await db.commit()

async def cleanup_expired_app_bans():
    """Удаляет истекшие запреты на заявки"""
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("DELETE FROM application_bans WHERE until_date IS NOT NULL AND until_date < datetime('now')")
        await db.commit()
...
async def cb_admin_active_bans(callback: CallbackQuery):
    ...
    # Удаляем истекшие баны перед показом
...
async def cb_admin_active_app_bans(callback: CallbackQuery):
    ...
    # Удаляем истекшие запреты перед показом
