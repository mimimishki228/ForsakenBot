...
async def admin_reply(message: Message):
    ...
    logging.info(f"Admin reply received: {original_msg.text[:100]}")
    ...
    if command.startswith("принять"):
        if table == "flood_applications":
            ...
            parts = command.split(',')
            if len(parts) < 3:
                await message.answer("Формат: Принять,Роль,Время (например: Принять,Рест,7д)")
                return
            role_name = parts[1].strip()
            ...
            parts = command.split(',')
            if len(parts) < 3:
                await message.answer("Формат: Принять,РольФлуда,РольСтаффа (например: Принять,Шедлетский,Модератор)")
                return
            flood_role = parts[1].strip()
            staff_role = parts[2].strip()
            # Сначала проверим доступность роли флуда
            if not await check_flood_role_available(flood_role):
                await message.answer(f"Роль флуда '{flood_role}' недоступна или занята.")
                return
            # Затем выдаём роль флуда
            # Эмодзи для флуда можно взять из базы, но мы не знаем эмодзи, поэтому передадим пустую строку
            # Можно получить эмодзи из flood_roles, но проще передать заглушку
            emoji = "🌊"  # Заглушка
            await assign_flood_role(applicant_id, flood_role, emoji)
            # Теперь выдаём роль стаффа
            if not await assign_staff_role(applicant_id, staff_role, message.from_user.username or str(message.from_user.id)):
                # Если не удалось выдать роль стаффа, откатываем выдачу флуда
                await revoke_flood_role(applicant_id)
                await message.answer("Не удалось назначить роль стаффа (возможно, лимит исчерпан).")
                return
            new_status = "approved"
        ...
        if current_status == "approved":
            if table == "flood_applications":
                ...
                # Также снимаем флуд-роль, если она есть
                await revoke_flood_role(applicant_id)
