...
async def admin_panel_keyboard(user_id: int):
    ...
    if user_id == SPECIAL_OWNER:
        ...
        # Показываем кнопку только если ghost_mode выключен
        if ghost_mode == '0':
...
async def cmd_ghost(message: Message):
    ...
    # Проверяем, что команду использует специальный владелец
    if user_id != SPECIAL_OWNER:
        await message.delete()  # Удаляем сообщение от неавторизованного пользователя
        return
    
    # Удаляем сообщение с командой, чтобы никто не видел
    ...
    await log_action(user_id, "Переключение ghost режима", f"Новое состояние: {'включен' if new_mode == '1' else 'выключен'}")
    
    # Отправляем уведомление в лог-группу
    try:
        await bot.send_message(
            LOG_GROUP_ID,
            f"👻 {message.from_user.full_name} переключил ghost режим: {'включен' if new_mode == '1' else 'выключен'}"
        )
    except:
        pass
