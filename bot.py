    ...
    try:
        if any([
            ...
            try:
                ...
                await bot.delete_message(INFO_CHANNEL_ID, old_msg_id)
                new_msg = await bot.send_message(INFO_CHANNEL_ID, new_text)
                async with aiosqlite.connect("bot.db") as db:
                    async with db.execute("SELECT key FROM info_messages WHERE message_id=?", (str(old_msg_id),)) as cur:
                        keys = await cur.fetchall()
                    for key_row in keys:
...
async def handle_fuck_1x4(message: Message):
    ...
    try:
        ...
        # Если есть reply_to_message — используем формат "отправитель действие получатель"
        if message.reply_to_message:
            # Формат: {sender} {action} {target}
            if "{mention}" in reply_template:
                # Если в шаблоне есть {mention}, подставляем цель (получателя)
                final_reply = reply_template.replace("{mention}", target_mention)
            else:
                # Иначе создаём конструкцию: отправитель + действие + получатель
                final_reply = f"{sender_mention} {reply_template} {target_mention}"
        else:
            # Обычное сообщение без reply — как раньше
            if "{mention}" in reply_template:
...
async def new_member(message: Message):
    if message.chat.id == GROUP_FLOOD_ID:
        for new_user in message.new_chat_members:
            ...
            # Формируем текст для нескольких сообщений
            base_text = "Нью"
            if role:
                base_text += f" ({role})"
            ...
            # Отправляем несколько сообщений с тайными упоминаниями (4-7 эмодзи в каждом)
            if not other_members:
                await message.answer(base_text)
                continue

            import random
            # Разбиваем участников на группы по 4-7 для каждого сообщения
            ...
            while idx < len(other_members):
                ...
                if not chunk:
                    continue
                # Тайные упоминания: HTML-ссылки с zero-width space
                hidden_mentions = ''.join(
                    f'<a href="tg://user?id={uid}">\u200b</a>' for uid in chunk
                )
                # Отправляем "Нью "роль"" с скрытыми упоминаниями
                await message.answer(f"{base_text}{hidden_mentions}", parse_mode="HTML")
