...
# ---------- Логирование ----------
async def add_log(action: str, user_id: int, target_id: int = None, details: str = ""):
    """Добавляет запись в лог"""
    timestamp = datetime.now().isoformat()
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT INTO logs (timestamp, action, user_id, target_id, details) VALUES (?, ?, ?, ?, ?)",
            (timestamp, action, user_id, target_id, details)
        )
        await db.commit()
...
async def get_logs(limit: int = 100) -> list:
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT timestamp, action, user_id, target_id, details FROM logs ORDER BY timestamp DESC LIMIT ?",
            (limit,)
        ) as cur:
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    ...
    await add_log("assign_flood_role", user_id, None, f"Роль: {role_name}, эмодзи: {emoji}")
...
async def revoke_flood_role(user_id: int):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_flood_info()
    await add_log("revoke_flood_role", user_id, None, "")
...
async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    ...
    await add_log("assign_rest_role", user_id, None, f"Роль: {role_name}, до: {expiry_date}")
...
async def revoke_rest_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_rest_info()
    await add_log("revoke_rest_role", user_id, None, f"Роль: {role_name}")
...
async def assign_staff_role(user_id: int, role_name: str, username: str):
    ...
    await add_log("assign_staff_role", user_id, None, f"Роль: {role_name}, username: {username}")
    return True
...
async def revoke_staff_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_staff_info()
    await add_log("revoke_staff_role", user_id, None, f"Роль: {role_name}")
...
async def set_cooldown(user_id: int, category: str, seconds: int):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await add_log("set_cooldown", user_id, None, f"Категория: {category}, секунд: {seconds}")
...
async def remove_cooldown(user_id: int, category: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await add_log("remove_cooldown", user_id, None, f"Категория: {category}")
...
async def set_cooldown_duration(category: str, seconds: int):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("INSERT OR REPLACE INTO cooldown_settings (category, duration_seconds) VALUES (?, ?)",
                         (category, seconds))
        await db.commit()
    await add_log("set_cooldown_duration", 0, None, f"Категория: {category}, секунд: {seconds}")
...
async def cmd_start(message: Message):
    ...
    await add_log("start_bot", user_id, None, "")
...
async def cb_admin_logs(callback: CallbackQuery):
    ...
    if not logs:
        await callback.message.edit_text("📋 Логи пусты.")
        return
    
    text = "📋 **Последние логи:**\n\n"
    for timestamp, action, user_id, target_id, details in logs:
        dt = datetime.fromisoformat(timestamp).strftime("%d.%m %H:%M")
        target_info = f" → {target_id}" if target_id else ""
        text += f"`{dt}` {action} {user_id}{target_info}"
        if details:
            text += f" ({details})"
        text += "\n"
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("apply_flood", user_id, None, "")
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
    await add_log("apply_rest", user_id, None, "")
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
    await add_log("apply_staff", user_id, None, "")
...
async def process_support_start(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("support_message", user_id, None, "")
...
async def admin_reply(message: Message):
    ...
    # Проверяем, не является ли админ автором заявки (для заявок)
    if "Заявка во флуд" in original_msg.text or "Заявка в рест" in original_msg.text or "Заявка в стафф" in original_msg.text:
        try:
            user_id_str = original_msg.text.split("User ID:")[1].split("\n")[0].strip()
            applicant_id = int(user_id_str)
            if applicant_id == admin_id:
                await message.answer("❌ Администратор не может принимать или отклонять свою собственную заявку.")
                return
        except:
            pass
    ...
    if "Новое обращение в поддержку" in original_msg.text or "Ответ от администратора" in original_msg.text:
        try:
            ...
            if user_id == admin_id:
                await message.answer("❌ Администратор не может отвечать самому себе в поддержке.")
                return
            ...
            await add_log("support_admin_reply", admin_id, user_id, "")
    ...
    if command.startswith("принять"):
        if table == "flood_applications":
            ...
            parts = command.split(',')
            if len(parts) < 3:
                await message.answer("Формат: Принять,Роль,Время")
                return
            role_name = parts[1].strip()
        ...
        await add_log(f"approve_{category}", admin_id, applicant_id, comment)
        ...
        await add_log(f"reject_{category}", admin_id, applicant_id, comment)
    else:
        await message.answer("Неверная команда.")

# ---------- Обработка ответов пользователя в диалоге поддержки ----------
@dp.message(F.text)
async def support_reply(message: Message):
    user_id = message.from_user.id
    
    # Проверка на команду удаления чата поддержки
        ...
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT 1 FROM support_dialogs WHERE user_id=? AND status='active'", (user_id,)) as cur:
                if await cur.fetchone():
                    ...
                    await db.commit()
                    await message.answer("✅ Чат поддержки закрыт. Вы можете написать новое обращение в любое время.")
                    await add_log("close_support", user_id, None, "")
                else:
                    await message.answer("У вас нет активного чата поддержки.")
        return
    ...
    await add_log("support_user_reply", user_id, None, "")
...
async def cb_my_cooldowns(callback: CallbackQuery):
    ...
    await add_log("view_cooldowns", user_id, None, "")
...
async def show_applications(chat_id: int, table: str, title: str):
    ...
    if not apps:
        ...
        for user_id, app_text, status in apps:
            ...
            if table == "flood_applications":
                format_text = "Формат ответа:\nПринять,Роль,Эмодзи\nили\nОтказать,Причина"
            elif table == "rest_applications":
                format_text = "Формат ответа:\nПринять,Роль,Время\nили\nОтказать,Причина"
            elif table == "staff_applications":
                format_text = "Формат ответа:\nПринять,@username,Роль\nили\nОтказать,Причина"
            else:
                format_text = ""
...
async def cb_admin_app_flood(callback: CallbackQuery):
    ...
    await add_log("view_applications", callback.from_user.id, None, "flood")
...
async def cb_admin_app_rest(callback: CallbackQuery):
    ...
    await add_log("view_applications", callback.from_user.id, None, "rest")
...
async def cb_admin_app_staff(callback: CallbackQuery):
    ...
    await add_log("view_applications", callback.from_user.id, None, "staff")
...
async def process_ban(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("ban_user", actor_id, target_id, "")
...
async def process_unban(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("unban_user", actor_id, target_id, "")
...
async def cb_toggle_flood(callback: CallbackQuery):
    ...
    await add_log("toggle_flood_apps", callback.from_user.id, None, f"статус: {new}")
...
async def cb_toggle_rest(callback: CallbackQuery):
    ...
    await add_log("toggle_rest_apps", callback.from_user.id, None, f"статус: {new}")
...
async def cb_toggle_staff(callback: CallbackQuery):
    ...
    await add_log("toggle_staff_apps", callback.from_user.id, None, f"статус: {new}")
...
async def process_flood_settings(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("update_flood_roles", message.from_user.id, None, "")
...
async def process_staff_settings(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("update_staff_roles", message.from_user.id, None, "")
...
async def process_delete_flood_role(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_flood_role", message.from_user.id, None, role_name)
...
async def process_delete_staff_role(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_staff_role", message.from_user.id, user_id, role_name)
...
async def process_delete_rest_role(message: Message, state: FSMContext):
    ...
    await message.answer(f"Роль '{role_name}' удалена.")
    await state.clear()
    await add_log("delete_rest_role", message.from_user.id, None, role_name)
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("add_admin", message.from_user.id, user_id, "")
...
async def process_remove_admin(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("remove_admin", message.from_user.id, user_id, "")
...
async def cb_admin_list_admins(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, parse_mode="Markdown")
    await add_log("view_admins", callback.from_user.id, None, "")
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("send_message", message.from_user.id, None, f"target: {target}")
...
async def process_delete_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("delete_message", message.from_user.id, None, f"target: {target}, msg_id: {msg_id}")
...
async def process_ban_app(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("ban_application", message.from_user.id, user_id, category)
...
async def process_allow_app(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("allow_application", message.from_user.id, user_id, category)
...
async def process_cooldown_give(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("give_cooldown", message.from_user.id, user_id, f"{category}: {seconds}с")
...
async def process_cooldown_remove(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("remove_cooldown_admin", message.from_user.id, user_id, category)
...
async def process_cooldown_settings(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("set_cooldown_duration_admin", message.from_user.id, None, f"{category}: {seconds}с")
...
async def process_template(message: Message, state: FSMContext):
    ...
    if category == "flood":
        ...
        await update_rest_info()
    await state.clear()
    await add_log("set_template", message.from_user.id, None, f"{category}")
...
async def process_bind_link(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("bind_info", message.from_user.id, None, f"{category}: {msg_id}")
...
async def cb_fuck_toggle(callback: CallbackQuery):
    ...
    await replace_menu(callback, "Настройка функции 'Ебать 1x4':", fuck_menu_keyboard())
    await add_log("toggle_fuck", callback.from_user.id, None, f"статус: {new}")
...
async def process_fuck_trigger(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("set_fuck_trigger", message.from_user.id, None, trigger)
...
async def process_fuck_reply(message: Message, state: FSMContext):
    ...
    await state.clear()
    await add_log("set_fuck_reply", message.from_user.id, None, reply)
...
async def cb_destroy_yes(callback: CallbackQuery):
    ...
    try:
        ...
        await add_log("destroy_group", callback.from_user.id, None, "")
    except Exception as e:
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await add_log("clear_history", callback.from_user.id, None, category)
