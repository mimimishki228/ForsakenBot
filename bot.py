...
async def get_user_pending_applications(user_id: int) -> list:
    """Возвращает список категорий, в которых у пользователя есть активные заявки (проверяет и статус, и таблицу)"""
    pending = []
    status = await get_user_status(user_id)
    
    # Проверяем статус пользователя
    if status == "pending_flood":
        # Дополнительно проверяем, есть ли запись в таблице
        async with aiosqlite.connect("bot.db") as db:
