...
# ---------- Работа с info_messages (одиночные сообщения) ----------
...
# ---------- Утилита для обновления меню (удаляет старое, отправляет новое) ----------
...
async def admin_reply(message: Message):
    ...
    # Проверяем, что это заявка (содержит User ID)
    if not original_msg.text or "User ID:" not in original_msg.text:
        return

    # Определяем тип заявки по заголовку
    ...
    if text.lower().startswith("принять"):
        ...
        if current_status == "rejected":
            ...
            return
        # Отзываем роль, если была выдана
        if current_status == "approved":
            if table == "flood_applications":
                await revoke_flood_role(applicant_id)
            elif table == "rest_applications":
                # Получаем роль из rest_roles
                async with aiosqlite.connect("bot.db") as db:
        ...
        # Статус пользователя сбрасываем, если был не pending
