...
def admin_panel_keyboard(user_id: int):
    # Определяем статус функции "выебать 1x4" для отображения на кнопке
    import sqlite3
    # Используем синхронный вызов для простоты, но лучше асинхронно – для краткости оставим так
    # В реальном коде лучше сделать async функцию, но здесь для примера.
    # Мы перенесём логику в обработчик, а здесь просто покажем статичный текст.
    # Вместо этого кнопка будет всегда показывать "Ебать 1x4 (вкл/выкл)" и менять состояние в колбэке.
    # Для динамического обновления текста мы можем каждый раз читать из БД, но проще сделать отдельный обработчик.
    # Поэтому здесь просто добавим кнопку с фиксированным текстом, который будет обновляться в колбэке.
    # Но чтобы не усложнять, сделаем кнопку с текстом "Ебать 1x4" и в колбэке будем менять статус и обновлять меню.
    # Поэтому в клавиатуре просто добавим кнопку, а текст будет меняться в обработчике.
    buttons = [
    ...
    if is_owner(user_id):
        ...
        # Новая кнопка: текст будет динамическим, но мы обновим его в обработчике при нажатии
        buttons.append([InlineKeyboardButton(text="🍆 Ебать 1x4 (переключить)", callback_data="admin_toggle_fuck")])
...
# ---------- Админ: переключение функции "выебать 1x4" ----------
@dp.callback_query(F.data == "admin_toggle_fuck")
async def cb_toggle_fuck(callback: CallbackQuery):
    if not await is_owner(callback.from_user.id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    async with aiosqlite.connect("bot.db") as db:
    ...
    # Обновляем админ-панель (перезапускаем меню)
    await replace_menu(callback, "Админ-панель:", admin_panel_keyboard(callback.from_user.id))
...
async def handle_fuck_1x4(message: Message):
    ...
    if "выебать 1x4" in text_lower:
        ...
        async with aiosqlite.connect("bot.db") as db:
            ...
            if enabled == '0':
                return
        # Получаем юзернейм автора (или ID, если нет юзернейма)
        user = message.from_user
        mention = f"@{user.username}" if user.username else f"[{user.full_name}](tg://user?id={user.id})"
        # Формируем ответ
        reply_text = f"1x4 был сладко выебан в очко ({mention})"
...
async def on_startup():
    async with aiosqlite.connect("bot.db") as db:
        ...
        # Инициализация настройки "выебать 1x4" (по умолчанию включена)
