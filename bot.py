...
# ---------- Заявки участника (без подтверждения) ----------
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки во флуд", f"Текст: {app_text[:50]}...")
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки в рест", f"Текст: {app_text[:50]}...")
    await message.answer("✅ Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки в стафф", f"Текст: {app_text[:50]}...")
    await message.answer("✅ Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
...
# ---------- Админ: просмотр заявок (без удаления старых) ----------
async def show_applications(chat_id: int, table: str, title: str, format_hint: str):
    ...
    if not apps:
        await bot.send_message(chat_id, f"Нет заявок: {title}")
        return
