...
async def replace_menu(callback: CallbackQuery, text: str, reply_markup: InlineKeyboardMarkup):
    ...
    message_id = callback.message.message_id    await bot.send_message(chat_id, text, reply_markup=reply_markup)
    try:
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    # Проверяем, есть ли активный диалог поддержки
    async with aiosqlite.connect("bot.db") as db:
...
async def admin_reply(message: Message):
    ...
    if command.startswith("принять"):
        ...
        # Если была принята, отзываем роль
        if current_status == "approved":
            if table == "flood_applications":
...
async def support_reply(message: Message):
    ...
    # Проверяем, есть ли активный диалог поддержки у пользователя
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT 1 FROM support_dialogs WHERE user_id=? AND status='active'", (user_id,)) as cur:
            if not await cur.fetchone():
                return
    
    # Проверяем, не является ли это ответом на сообщение админа (это обрабатывается в admin_reply)
    if message.reply_to_message:
        return
    
    # Проверяем, не является ли пользователь админом (админы не должны отправлять сообщения в поддержку через этот обработчик)
...
async def cb_admin_list_admins(callback: CallbackQuery):
    ...
    # Админы (исключаем владельцев, чтобы не дублировать)
...
# ---------- Обработчик сообщений в группе флуда: "выебать 1x4" (работает для всех участников) ----------
