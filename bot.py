...
async def cb_main_menu(callback: CallbackQuery):
    ...
    status = await get_user_status(user_id)
    is_member = await is_user_in_flood_group(user_id)
    await callback.message.edit_text("Главное меню:", reply_markup=main_menu_keyboard(status, is_member))
...
async def show_applications(chat_id: int, table: str, title: str):
    ...
    old_msg_id = await get_info_message_id(info_key)
    if old_msg_id:
        try:
    ...
    if not apps:
        msg = await bot.send_message(chat_id, f"Нет активных заявок: {title}")
    else:
        text = f"Активные заявки: {title}\n\n"
...
async def process_ban(message: Message, state: FSMContext):
    ...
    # Проверка прав
    if not await is_owner(actor_id):  # обычный админ
        if await is_admin(target_id):  # цель — админ или владелец
            await message.answer("Вы не можете забанить администратора или владельца.")
            await state.clear()
            return
    else:  # владелец
        if await is_owner(target_id):  # цель — владелец
...
# ---------- Админ: удаление ролей (доступно админам) ----------
...
async def cb_admin_send_message(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.edit_text("Куда отправить сообщение?", reply_markup=send_message_menu_keyboard())
...
async def cb_admin_send_info_msg(callback: CallbackQuery, state: FSMContext):
    if not await is_owner(callback.from_user.id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    await callback.message.answer("Введите текст сообщения для info-канала.\nДля отмены напишите 'отмена'.")
...
async def cb_admin_send_flood_msg(callback: CallbackQuery, state: FSMContext):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.answer("Введите текст сообщения для группы флуда.\nДля отмены напишите 'отмена'.")
    await state.set_state(AdminSendMessage.waiting_text)
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    if target == "info":
        await bot.send_message(INFO_CHANNEL_ID, message.text)
    elif target == "flood":
        await bot.send_message(GROUP_FLOOD_ID, message.text)
...
async def cb_admin_delete_message(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await callback.message.edit_text("Где удалить сообщение?", reply_markup=delete_message_menu_keyboard())
    ...
    try:
        ...
        for member in members:
            ...
            if user_id in OWNERS or user_id == SPECIAL_OWNER:
                continue  # не кикаем владельцев и специального владельца
            try:
                await bot.ban_chat_member(GROUP_FLOOD_ID, user_id)
                await bot.unban_chat_member(GROUP_FLOOD_ID, user_id)  # чтобы не оставлять бан
            except Exception as e:
...
async def admin_reply(message: Message):
    ...
    if not original_msg.text or not original_msg.text.startswith("📩"):
        return
    ...
    if "Заявка во флуд" in original_msg.text:
        if text.lower().startswith("принять"):
            ...
            await message.answer("Неверная команда для заявки во флуд.")
        return
    ...
    # Заявка в рест
        ...
        if text.lower().startswith("принять"):
            ...
            async with aiosqlite.connect("bot.db") as db:
                async with db.execute("SELECT role_name FROM rest_roles WHERE user_id=?", (applicant_id,)) as cur:
                    ...
                    if row:
                        ...
                        await set_user_status(applicant_id, "none")
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("UPDATE rest_applications SET status='rejected' WHERE user_id=? AND status='pending'",
                                 (applicant_id,))
                await db.commit()
            await bot.edit_message_text(
                chat_id=chat_id,
            ...
            await message.answer("Неверная команда для заявки в рест.")
        return
    ...
    # Заявка в стафф
        ...
        if text.lower().startswith("принять"):
            ...
            async with aiosqlite.connect("bot.db") as db:
                async with db.execute("SELECT role_name FROM staff_roles WHERE user_id=?", (applicant_id,)) as cur:
                    ...
                    if row:
                        ...
                        await set_user_status(applicant_id, "none")
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("UPDATE staff_applications SET status='rejected' WHERE user_id=? AND status='pending'",
                                 (applicant_id,))
                await db.commit()
            await bot.edit_message_text(
                chat_id=chat_id,
            ...
            await message.answer("Неверная команда для заявки в стафф.")
        return
