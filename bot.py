...
def user_panel_keyboard(status: str):
    buttons = []
    if status == "flood":
...
async def cmd_start(message: Message):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    status = await get_user_status(user_id)
    await message.answer("Добро пожаловать! Выберите раздел:", reply_markup=main_menu_keyboard(status))

@dp.callback_query(F.data == "main_menu")
async def cb_main_menu(callback: CallbackQuery):
    user_id = callback.from_user.id
    status = await get_user_status(user_id)
    await callback.message.edit_text("Главное меню:", reply_markup=main_menu_keyboard(status))
    ...
    # Заявка в рест
        ...
        if text.startswith("Принять"):
            ...
            try:
                ...
                await message.answer("Не удалось определить ID заявителя.")
                return
            # Проверяем, если уже имеет роль рест
            async with aiosqlite.connect("bot.db") as db:
    ...
    # Заявка в стафф
        ...
        if text.startswith("Принять"):
            ...
            try:
                ...
                await message.answer("Не удалось определить ID заявителя.")
                return
            # Ищем роль стаффа у пользователя
            async with aiosqlite.connect("bot.db") as db:
