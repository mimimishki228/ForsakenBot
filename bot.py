...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    await state.set_state(FloodForm.waiting_edit)  # временно, чтобы не конфликтовать
...
async def app_edit(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FloodForm.waiting_edit)  # используем единое состояние для всех категорий, но потом переключим
    # Определяем, какое состояние использовать в зависимости от категории
    if category == "flood":
...
async def flood_edit(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        ...
        await state.set_state(FloodForm.waiting_edit)  # временно
        data = await state.get_data()
        current_text = data.get("application_text", "")
        await message.answer(
            "Вы уверены, что хотите отправить заявку?",
    ...
    for admin_id in admin_list:
        try:
            text = (
                ...
                f"📝 Сообщение:\n{message.text}\n\n"
                f"Ответьте на это сообщение, чтобы отправить ответ пользователю, или используйте /ticket {ticket_id} в админ-панели."
            )
...
async def cb_admin_tickets(callback: CallbackQuery):
    ...
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data="panel_admin")])
...
async def process_ticket_reply(message: Message, state: FSMContext):
    ...
    await cb_ticket_view(message)  # Имитируем callback
...
# ---------- Обработка ответов админа через reply (старый способ) ----------
...
async def admin_reply(message: Message):
    ...
    # Проверяем, не является ли это ответом на тикет (сообщение от бота о новом тикете)
    if "Новый тикет!" in original_msg.text:
        try:
            ...
            # Сохраняем ответ как сообщение в тикете
            admin_id = message.from_user.id
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("INSERT INTO support_messages (ticket_id, sender_id, text, timestamp, is_from_admin) VALUES (?, ?, ?, ?, 1)",
                                 (ticket_id, admin_id, message.text, datetime.now().isoformat()))
    ...
    if command.startswith("принять"):
        if table == "flood_applications":
            ...
            parts = command.split(',')
            if len(parts) < 2:
                await message.answer("Формат: Принять,время (например: Принять,7д)")
                return
            ...
            parts = command.split(',')
            if len(parts) < 2:
                await message.answer("Формат: Принять,роль (например: Принять,Модератор)")
                return
            role_name = parts[1].strip()
...
async def cb_admin_app_rest(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "rest_applications", "Заявка в рест", "Формат: Принять,время (например: Принять,7д)\nили Отказать,Причина")
...
async def cb_admin_app_staff(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "staff_applications", "Заявка в стафф", "Формат: Принять,роль (например: Принять,Модератор)\nили Отказать,Причина")
