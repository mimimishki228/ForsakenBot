...
async def log_action(user_id: int, action: str, details: str = ""):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    try:
        # Получаем имя пользователя для красивого логирования
        try:
...
async def get_user_pending_applications(user_id: int) -> list:
    """Возвращает список категорий, в которых у пользователя есть активные заявки"""
    pending = []
    status = await get_user_status(user_id)
...
async def delete_application(user_id: int, category: str):
    """Удаляет заявку пользователя в указанной категории"""
...
async def check_user_has_application(user_id: int, category: str) -> bool:
    """Проверяет, есть ли у пользователя активная заявка в конкретной категории"""
    table = f"{category}_applications"
    async with aiosqlite.connect("bot.db") as db:
...
async def cmd_start(message: Message):
    ...
    await log_action(user_id, "Команда /start", "")
...
async def cb_main_menu(callback: CallbackQuery):
    ...
    await log_action(user_id, "Переход в главное меню", "")
    status = await get_user_status(user_id)
    is_member = await is_user_in_flood_group(user_id)
...
async def cb_panel_user(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл панель участника", "")
    if await is_banned(user_id):
...
async def cb_get_flood_link(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Запросил ссылку на группу флуда", "")
...
async def cb_admin_logs(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр логов", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
    async with aiosqlite.connect("bot.db") as db:
...
async def cb_apply_flood(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало подачи заявки во флуд", "")
    ...
    # Проверяем, есть ли уже активная заявка в ЛЮБОЙ категории
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        await state.clear()
        return
    await log_action(user_id, "Написал текст заявки во флуд", f"Текст: {message.text[:50]}...")
...
async def cb_apply_rest(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало подачи заявки в рест", "")
    if await is_user_in_application_process(state):
        await callback.answer("Вы уже пишете заявку, завершите её или отмените.", show_alert=True)
        return
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        await state.clear()
        return
    await log_action(user_id, "Написал текст заявки в рест", f"Текст: {message.text[:50]}...")
    await state.update_data(application_text=message.text, edit_count=0)
    await message.answer(
        "Вы уверены, что хотите отправить заявку?",
...
async def cb_apply_staff(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало подачи заявки в стафф", "")
    if await is_user_in_application_process(state):
        await callback.answer("Вы уже пишете заявку, завершите её или отмените.", show_alert=True)
        return
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        await state.clear()
        return
    await log_action(user_id, "Написал текст заявки в стафф", f"Текст: {message.text[:50]}...")
    await state.update_data(application_text=message.text, edit_count=0)
    await message.answer(
        "Вы уверены, что хотите отправить заявку?",
...
async def app_edit(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[2]
    user_id = callback.from_user.id
    await log_action(user_id, f"Редактирование заявки в {category}", "")
    data = await state.get_data()
...
# Обработчики редактирования для каждой категории
...
async def flood_edit(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        return
    await log_action(user_id, "Обновление текста заявки во флуд", f"Новый текст: {message.text[:50]}...")
...
async def rest_edit(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        return
    await log_action(user_id, "Обновление текста заявки в рест", f"Новый текст: {message.text[:50]}...")
    await state.update_data(application_text=message.text)
    await message.answer(
        "Текст обновлён. Подтвердите отправку:",
        reply_markup=application_confirm_keyboard("rest")
...
async def staff_edit(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        ...
        return
    await log_action(user_id, "Обновление текста заявки в стафф", f"Новый текст: {message.text[:50]}...")
    await state.update_data(application_text=message.text)
    await message.answer(
        "Текст обновлён. Подтвердите отправку:",
        reply_markup=application_confirm_keyboard("staff")
...
async def app_delete(callback: CallbackQuery):
    """Обработчик нажатия на кнопку удаления заявки"""
    ...
    user_id = callback.from_user.id
    await log_action(user_id, f"Начало удаления заявки в {category}", "")
...
async def confirm_delete(callback: CallbackQuery):
    ...
    category = data_parts[2]
    user_id = callback.from_user.id
    await log_action(user_id, f"Подтверждение удаления заявки в {category}", "")
    ...
    await log_action(user_id, f"Удаление заявки в {category}", "Успешно")
...
async def cancel_delete(callback: CallbackQuery):
    ...
    user_id = callback.from_user.id
    await log_action(user_id, "Отмена удаления заявки", "")
    status = await get_user_status(user_id)
    is_member = await is_user_in_flood_group(user_id)
    
    # Определяем категорию из текста сообщения
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало создания тикета", "")
...
async def cb_user_ticket(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр активного тикета", "")
    async with aiosqlite.connect("bot.db") as db:
...
async def cb_admin_tickets(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр тикетов", "")
    if not await is_admin(user_id):
...
async def cb_ticket_view(callback: CallbackQuery):
    ...
    await log_action(user_id, f"Просмотр тикета #{ticket_id}", "")
    async with aiosqlite.connect("bot.db") as db:
...
async def cb_ticket_reply(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    ticket_id = int(callback.data.split("_")[2])
    await log_action(user_id, f"Начало ответа в тикет #{ticket_id}", "")
...
async def process_ticket_reply(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "отмена":
        await log_action(admin_id, "Отмена ответа в тикет", "")
...
async def admin_application_reply(message: Message):
    ...
    try:
        ...
        if applicant_id == admin_id:
            await log_action(admin_id, "Попытка обработать свою заявку", f"Категория {category}")
        ...
        if cmd_lower.startswith("принять"):
            await log_action(admin_id, f"Принятие {action_name}", f"Пользователь {applicant_id}")
            ...
            await log_action(admin_id, f"Отклонение {action_name}", f"Пользователь {applicant_id}")
            command = "отказать"
...
async def cb_my_cooldowns(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр своих кулдаунов", "")
...
async def cb_admin_applications(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр заявок", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_app_flood(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр заявок во флуд", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_app_rest(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр заявок в рест", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_app_staff(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр заявок в стафф", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_bans(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню банов", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_ban(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало бана пользователя", "")
...
async def process_ban(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(actor_id, "Бан пользователя", f"Забанен {target_id}")
...
async def cb_admin_unban(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало разбана пользователя", "")
...
async def process_unban(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(actor_id, "Разбан пользователя", f"Разбанен {target_id}")
...
async def cb_admin_toggle_apps(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню переключения заявок", "")
...
async def cb_toggle_flood(callback: CallbackQuery):
    ...
    await log_action(user_id, f"Переключение заявок во флуд", f"Новое состояние: {'открыты' if new == '1' else 'закрыты'}")
...
async def cb_toggle_rest(callback: CallbackQuery):
    ...
    await log_action(user_id, f"Переключение заявок в рест", f"Новое состояние: {'открыты' if new == '1' else 'закрыты'}")
...
async def cb_toggle_staff(callback: CallbackQuery):
    ...
    await log_action(user_id, f"Переключение заявок в стафф", f"Новое состояние: {'открыты' if new == '1' else 'закрыты'}")
...
async def cb_admin_settings(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл настройки инфо", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_admin_settings_flood(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало настройки ролей флуда", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def process_flood_settings(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки ролей флуда", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    text = message.text
...
async def cb_admin_settings_staff(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало настройки ролей стаффа", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def process_staff_settings(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки ролей стаффа", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    text = message.text
...
async def cb_admin_delete_roles(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню удаления ролей", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_delete_flood_role(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления роли флуда", "")
...
async def process_delete_flood_role(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена удаления роли флуда", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_delete_staff_role(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления роли стаффа", "")
...
async def process_delete_staff_role(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена удаления роли стаффа", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_delete_rest_role(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления рест-роли", "")
...
async def process_delete_rest_role(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена удаления рест-роли", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    role_name = message.text.strip()
...
async def cb_admin_manage_admins(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл управление админами", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_admin_add_admin(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало назначения админа", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def process_add_admin(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена назначения админа", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    try:
...
async def cb_admin_remove_admin(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления админа", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def process_remove_admin(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена удаления админа", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    try:
...
async def cb_admin_list_admins(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Просмотр списка админов", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_admin_send_message(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню отправки сообщения", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_send_info_msg(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало отправки сообщения в info-канал", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_admin_send_flood_msg(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало отправки сообщения в группу флуда", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def process_send_message(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена отправки сообщения", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_admin_delete_message(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню удаления сообщения", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_admin_delete_info_msg(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления сообщения из info-канала", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_admin_delete_flood_msg(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало удаления сообщения из группы флуда", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def process_delete_message(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена удаления сообщения", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_admin_app_bans(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню запретов на заявки", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_ban_app_category(callback: CallbackQuery, state: FSMContext):
    ...
    user_id = callback.from_user.id
    await log_action(user_id, f"Начало запрета на заявки в {category}", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def process_ban_app(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена запрета на заявки", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    data = await state.get_data()
    ...
    try:
        ...
        await message.answer("Неверный ID. Введите число.")
        return
    await log_action(user_id, f"Запрет на заявки в {category}", f"Пользователь {target_id}")
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("INSERT OR REPLACE INTO application_bans (user_id, category) VALUES (?, ?)", (target_id, category))
        await db.commit()
    await message.answer(f"Пользователю {target_id} запрещено подавать заявки в категорию '{category}'.")
    await state.clear()
...
async def cb_admin_allow_apps(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню разрешения заявок", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_allow_app_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[-1]
    user_id = callback.from_user.id
    await log_action(user_id, f"Начало снятия запрета на заявки в {category}", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def process_allow_app(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена снятия запрета на заявки", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    data = await state.get_data()
...
async def cb_admin_cooldown(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню управления кд", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_cooldown_give(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало выдачи кд", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_cooldown_remove(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Начало снятия кд", "")
    if not await is_admin(user_id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def cb_cooldown_give_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[-1]
    user_id = callback.from_user.id
    await log_action(user_id, f"Выдача кд в категории {category}", "")
    await state.update_data(category=category)
    await callback.message.answer(
...
async def process_cooldown_give(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена выдачи кд", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    try:
...
async def cb_cooldown_remove_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[-1]
    user_id = callback.from_user.id
    await log_action(user_id, f"Снятие кд в категории {category}", "")
    await state.update_data(category=category)
...
async def process_cooldown_remove(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена снятия кд", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    try:
...
async def cb_admin_cooldown_settings(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл настройки кд", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_cooldown_settings_category(callback: CallbackQuery, state: FSMContext):
    category = callback.data.split("_")[-1]
    user_id = callback.from_user.id
    await log_action(user_id, f"Настройка кд для {category}", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    await state.update_data(category=category)
...
async def process_cooldown_settings(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки кд", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    try:
...
async def cb_admin_templates(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню шаблонов", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_template_category(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    category = callback.data.split("_")[1]
    await log_action(user_id, f"Настройка шаблона для {category}", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    await state.update_data(category=category)
...
async def process_template(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки шаблона", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_admin_bind_info(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню привязки info", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
...
async def cb_bind_category(callback: CallbackQuery, state: FSMContext):
    ...
    await log_action(user_id, f"Привязка info для {category}", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
    await state.update_data(category=category)
...
async def process_bind_link(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена привязки info", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
    link = message.text.strip()
...
async def cb_admin_fuck_menu(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню 1x4", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
...
async def cb_fuck_toggle(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Toggle 1x4", f"Новое состояние: {new}")
...
async def cb_fuck_settings(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл настройки 1x4", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
...
async def cb_fuck_menu(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Вернулся в меню 1x4", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
    await replace_menu(callback, "Настройка функции 'Ебать 1x4':", fuck_menu_keyboard())
...
async def cb_fuck_set_trigger(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Настройка триггера 1x4", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
    await callback.message.answer(
...
async def process_fuck_trigger(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки триггера 1x4", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_fuck_set_reply(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    await log_action(user_id, "Настройка ответа 1x4", "")
    if not await is_admin(user_id):
        await callback.answer("Недостаточно прав", show_alert=True)
        return
    await callback.message.answer(
...
async def process_fuck_reply(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text and message.text.lower() == "отмена":
        await log_action(user_id, "Отмена настройки ответа 1x4", "")
        await message.answer("Действие отменено.")
        await state.clear()
        return
...
async def cb_admin_destroy_confirm(callback: CallbackQuery):
    ...
    if user_id != SPECIAL_OWNER:
        await callback.answer("Нет прав", show_alert=True)
        return
    await log_action(user_id, "Открыл подтверждение уничтожения", "")
...
async def cb_destroy_yes(callback: CallbackQuery):
    ...
    if user_id != SPECIAL_OWNER:
        await callback.answer("Нет прав", show_alert=True)
        return
    await log_action(user_id, "Начало уничтожения группы", "")
...
async def cb_destroy_no(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Отмена уничтожения", "")
...
async def cb_admin_clear_history(callback: CallbackQuery):
    user_id = callback.from_user.id
    await log_action(user_id, "Открыл меню очистки истории", "")
    if not await is_owner(user_id):
        await callback.answer("Только для владельцев", show_alert=True)
        return
