...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки во флуд", f"Текст: {text[:50]}...")
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки в рест", f"Текст: {text[:50]}...")
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(user_id, "Подача заявки в стафф", f"Текст: {text[:50]}...")
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await state.clear()
...
async def process_support_start(message: Message, state: FSMContext):
    ...
    await log_action(user_id, "Обращение в поддержку", f"Текст: {message.text[:50]}...")
    await message.answer("✅ Ваше обращение отправлено администраторам. Ожидайте ответа.")
    await apply_auto_cooldown(user_id, "support")
    await state.clear()

# ---------- Закрытие чата поддержки пользователем ----------
@dp.message(StateFilter(None), F.text)  # <-- добавлен фильтр
async def support_close_command(message: Message):
    ...
    if message.text.lower() == "удалить чат поддержки":
        async with aiosqlite.connect("bot.db") as db:
            await db.execute("UPDATE support_dialogs SET status='closed' WHERE user_id=? AND status='active'", (user_id,))
            await db.commit()
        await message.answer("✅ Чат поддержки закрыт. Вы можете снова написать в поддержку.")
        await log_action(user_id, "Закрыл чат поддержки", "")
...
async def admin_reply(message: Message):
    ...
    if "Новое обращение в поддержку" in original_msg.text or "Ответ от администратора" in original_msg.text:
        try:
            ...
            await log_action(admin.id, "Ответ в поддержку", f"Пользователю {user_id}: {message.text[:50]}...")
    ...
    # Проверка: админ не может принимать/отклонять свою заявку
    ...
    if command.startswith("принять"):
        if table == "flood_applications":
            ...
            role_name = "Рест"  # Можно задать дефолтное имя
...
async def support_reply(message: Message):
    ...
    # Пропускаем команду закрытия чата
    ...
    if message.reply_to_message:
        # Если пользователь отвечает на сообщение админа, это считается ответом в диалоге
        pass
    ...
    await log_action(user_id, "Ответ в поддержку", f"Текст: {message.text[:50]}...")
