...
# ---------- Логирование ----------
async def add_log(action: str, user_id: int = None, target_id: int = None, details: str = "", admin_id: int = None):
    ...
    if target_id:
        log_entry += f" | Цель: {target_id}"
    if admin_id:
        log_entry += f" | Админ: {admin_id}"
    if details:
        log_entry += f" | {details}"
    
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT INTO logs (timestamp, action, user_id, target_id, admin_id, details) VALUES (?, ?, ?, ?, ?, ?)",
            (timestamp, action, user_id, target_id, admin_id, details)
        )
        await db.commit()
...
async def get_logs(limit: int = 100) -> list:
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute(
            "SELECT timestamp, action, user_id, target_id, admin_id, details FROM logs ORDER BY timestamp DESC LIMIT ?",
            (limit,)
        ) as cur:
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    ...
    await add_log("assign_flood_role", user_id=user_id, details=f"Роль: {role_name}, эмодзи: {emoji}")
...
async def revoke_flood_role(user_id: int):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_flood_info()
    await add_log("revoke_flood_role", user_id=user_id)
...
async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    ...
    await add_log("assign_rest_role", user_id=user_id, details=f"Роль: {role_name}, до: {expiry_date}")
...
async def revoke_rest_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_rest_info()
    await add_log("revoke_rest_role", user_id=user_id, details=f"Роль: {role_name}")
...
async def assign_staff_role(user_id: int, role_name: str, username: str):
    ...
    await add_log("assign_staff_role", user_id=user_id, details=f"Роль: {role_name}, username: {username}")
    return True
...
async def revoke_staff_role(user_id: int, role_name: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await update_staff_info()
    await add_log("revoke_staff_role", user_id=user_id, details=f"Роль: {role_name}")
...
def logs_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📜 Посмотреть логи", callback_data="view_logs")],
...
async def cb_admin_logs(callback: CallbackQuery):
    if not await is_owner(callback.from_user.id):
        ...
        return
    await replace_menu(callback, "📜 Управление логами:", logs_keyboard())
...
async def cb_view_logs(callback: CallbackQuery):
    ...
    if not logs:
        await callback.message.edit_text("📜 Логов пока нет.")
        return
    
    text = "📜 **Последние логи:**\n\n"
    for log in logs:
        timestamp, action, user_id, target_id, admin_id, details = log
        line = f"[{timestamp[:19]}] {action}"
        if user_id:
            line += f" | Пользователь: {user_id}"
        if target_id:
            line += f" | Цель: {target_id}"
        if admin_id:
            line += f" | Админ: {admin_id}"
        if details:
            line += f" | {details}"
        text += line + "\n"
...
async def cb_clear_logs(callback: CallbackQuery):
    ...
    await replace_menu(callback, "📜 Логи очищены.", admin_panel_keyboard(callback.from_user.id))
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    await add_log("apply_flood", user_id=user_id, details=f"Текст: {text[:50]}...")
    await state.clear()
...
async def process_rest_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await add_log("apply_rest", user_id=user_id, details=f"Текст: {text[:50]}...")
    await state.clear()
...
async def process_staff_application(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await message.answer("Ваша заявка отправлена. Ожидайте решения.")
    await add_log("apply_staff", user_id=user_id, details=f"Текст: {text[:50]}...")
    await state.clear()
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT 1 FROM support_dialogs WHERE user_id=? AND status='active'", (user_id,)) as cur:
    ...
    await callback.message.answer(
        ...
        "Для отмены напишите 'отмена'.\n"
        "Чтобы закрыть диалог, напишите 'удалить чат поддержки'."
    )
...
async def process_support_start(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect("bot.db") as db:
        await db.execute(
            "INSERT INTO support_dialogs (user_id, status, created_at) VALUES (?, 'active', ?)",
            (user_id, created_at)
        )
        await db.commit()
    ...
    await add_log("support_start", user_id=user_id, details=f"Сообщение: {message.text[:50]}...")
    await state.clear()

# ---------- Обработка сообщений пользователя (закрытие диалога и ответы) ----------
@dp.message(F.text)
async def user_messages_handler(message: Message):
    user_id = message.from_user.id
    ...
    # Если пользователь хочет закрыть диалог
    if message.text and message.text.lower() == "удалить чат поддержки":
        async with aiosqlite.connect("bot.db") as db:
            await db.execute("UPDATE support_dialogs SET status='closed' WHERE user_id=?", (user_id,))
            await db.commit()
        await message.answer("✅ Чат поддержки закрыт. Вы можете создать новый диалог через кнопку 'Поддержка' в панели участника.")
        await add_log("support_close", user_id=user_id)
        return
    
    # Если это ответ на сообщение от администратора (бот отправляет сообщения админов)
    if message.reply_to_message and message.reply_to_message.from_user.id == bot.id:
        ...
        for admin_id in admin_list:
            ...
            try:
                text = (
                    f"📩 **Ответ пользователя в диалоге поддержки!**\n\n"
                    f"👤 От: {mention}\n"
                    f"🆔 ID: {user_id}\n\n"
                    f"📝 Сообщение:\n{message.text}\n\n"
                    f"Ответьте на это сообщение, чтобы отправить ответ пользователю."
        ...
        await add_log("support_user_reply", user_id=user_id, details=f"Сообщение: {message.text[:50]}...")
        return
    
    # Если просто сообщение в диалоге (не ответ на админа)
    admin_list = []
    async with aiosqlite.connect("bot.db") as db:
        async with db.execute("SELECT user_id FROM admins") as cur:
            admin_rows = await cur.fetchall()
    ...
    await add_log("support_user_message", user_id=user_id, details=f"Сообщение: {message.text[:50]}...")

# ---------- Обработка ответов админа (поддержка и заявки) ----------
...
async def admin_reply(message: Message):
    ...
    # Обработка ответа на сообщение пользователя в поддержке
    if "Новое обращение в поддержку" in original_msg.text or "Новое сообщение в диалоге поддержки" in original_msg.text or "Ответ пользователя в диалоге поддержки" in original_msg.text:
        try:
            ...
            # Проверяем, не является ли админ тем же пользователем
            if message.from_user.id == user_id:
                await message.answer("❌ Вы не можете отвечать на свои собственные обращения в поддержку.")
                return
            
            async with aiosqlite.connect("bot.db") as db:
                async with db.execute("SELECT status FROM support_dialogs WHERE user_id=?", (user_id,)) as cur:
                    row = await cur.fetchone()
            ...
            await add_log("support_admin_reply", admin_id=message.from_user.id, target_id=user_id, details=f"Ответ: {message.text[:50]}...")
    ...
    # Пропускаем, если это ответ на свою заявку
    try:
        user_id_str = original_msg.text.split("User ID:")[1].split("\n")[0].strip()
        applicant_id = int(user_id_str)
        if message.from_user.id == applicant_id:
            await message.answer("❌ Вы не можете принимать или отклонять свою собственную заявку.")
            return
    except:
        pass
    ...
    if command.startswith("принять"):
        if table == "flood_applications":
            parts = command.split(',')
            if len(parts) < 3:
                await message.answer(f"Формат: {format_hint}")
                return
            ...
            if await check_flood_role_available(role_name):
                ...
                await add_log("approve_flood", admin_id=message.from_user.id, target_id=applicant_id, details=f"Роль: {role_name}")
            else:
            ...
            await add_log("approve_rest", admin_id=message.from_user.id, target_id=applicant_id, details=f"Роль: {role_name}")
            ...
            if await assign_staff_role(applicant_id, role_name, username):
                ...
                await add_log("approve_staff", admin_id=message.from_user.id, target_id=applicant_id, details=f"Роль: {role_name}")
            else:
        ...
        await add_log(f"reject_{category}", admin_id=message.from_user.id, target_id=applicant_id, details=f"Причина: {comment}")
    else:
        await message.answer(f"Неверная команда. Используйте формат:\n{format_hint}")
...
async def cb_admin_app_flood(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "flood_applications", "Заявка во флуд", "Принять,Роль,Эмодзи\nили\nОтказать,Причина")
...
async def cb_admin_app_rest(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "rest_applications", "Заявка в рест", "Принять,Роль,Время\nили\nОтказать,Причина")
...
async def cb_admin_app_staff(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "staff_applications", "Заявка в стафф", "Принять,@username,Роль\nили\nОтказать,Причина")
...
async def process_ban(message: Message, state: FSMContext):
    ...
    await add_log("ban_user", admin_id=actor_id, target_id=target_id)
    await state.clear()
...
async def process_unban(message: Message, state: FSMContext):
    ...
    await add_log("unban_user", admin_id=actor_id, target_id=target_id)
    await state.clear()
...
async def cb_toggle_flood(callback: CallbackQuery):
    ...
    await add_log("toggle_flood", admin_id=callback.from_user.id, details=f"Статус: {'открыты' if new == '1' else 'закрыты'}")
...
async def cb_toggle_rest(callback: CallbackQuery):
    ...
    await add_log("toggle_rest", admin_id=callback.from_user.id, details=f"Статус: {'открыты' if new == '1' else 'закрыты'}")
...
async def cb_toggle_staff(callback: CallbackQuery):
    ...
    await add_log("toggle_staff", admin_id=callback.from_user.id, details=f"Статус: {'открыты' if new == '1' else 'закрыты'}")
...
async def process_flood_settings(message: Message, state: FSMContext):
    ...
    await add_log("settings_flood", admin_id=message.from_user.id, details=f"Количество ролей: {len(roles)}")
    await state.clear()
...
async def process_staff_settings(message: Message, state: FSMContext):
    ...
    await add_log("settings_staff", admin_id=message.from_user.id, details=f"Количество ролей: {len(roles)}")
    await state.clear()
...
async def process_delete_flood_role(message: Message, state: FSMContext):
    ...
    await add_log("delete_flood_role", admin_id=message.from_user.id, details=f"Роль: {role_name}")
    await state.clear()
...
async def process_delete_staff_role(message: Message, state: FSMContext):
    ...
    await add_log("delete_staff_role", admin_id=message.from_user.id, target_id=user_id, details=f"Роль: {role_name}")
    await state.clear()
...
async def process_delete_rest_role(message: Message, state: FSMContext):
    ...
    await update_rest_info()
    await message.answer(f"Роль '{role_name}' удалена.")
    await add_log("delete_rest_role", admin_id=message.from_user.id, details=f"Роль: {role_name}")
    await state.clear()
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    await add_log("add_admin", admin_id=message.from_user.id, target_id=user_id)
    await state.clear()
...
async def process_remove_admin(message: Message, state: FSMContext):
    ...
    await add_log("remove_admin", admin_id=message.from_user.id, target_id=user_id)
    await state.clear()
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    if any([
        ...
        try:
            ...
            await add_log(f"send_{target}_media", admin_id=message.from_user.id)
        except Exception as e:
        ...
        await add_log(f"send_{target}_message", admin_id=message.from_user.id, details=f"Текст: {text[:50]}...")
...
async def process_delete_message(message: Message, state: FSMContext):
    ...
    try:
        ...
        await add_log(f"delete_{target}_message", admin_id=message.from_user.id, details=f"ID сообщения: {msg_id}")
    except Exception as e:
...
async def process_ban_app(message: Message, state: FSMContext):
    ...
    await add_log(f"ban_app_{category}", admin_id=message.from_user.id, target_id=user_id)
    await state.clear()
...
async def process_allow_app(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("DELETE FROM application_bans WHERE user_id=? AND category=?", (user_id, category))
        await db.commit()
    await message.answer(f"Пользователю {user_id} разрешено подавать заявки в категорию '{category}'.")
    await add_log(f"allow_app_{category}", admin_id=message.from_user.id, target_id=user_id)
    await state.clear()
...
async def process_cooldown_give(message: Message, state: FSMContext):
    ...
    await add_log(f"cooldown_give_{category}", admin_id=message.from_user.id, target_id=user_id, details=f"Длительность: {duration_readable}")
    await state.clear()
...
async def process_cooldown_remove(message: Message, state: FSMContext):
    ...
    await add_log(f"cooldown_remove_{category}", admin_id=message.from_user.id, target_id=user_id)
    await state.clear()
...
async def process_cooldown_settings(message: Message, state: FSMContext):
    ...
    await add_log("cooldown_settings", admin_id=message.from_user.id, details=f"Категория: {category}, длительность: {duration_readable}")
    await state.clear()
...
async def process_template(message: Message, state: FSMContext):
    ...
    if category == "flood":
        ...
        await update_rest_info()
    await add_log("template_update", admin_id=message.from_user.id, details=f"Категория: {category}")
    await state.clear()
...
async def process_bind_link(message: Message, state: FSMContext):
    ...
    await add_log("bind_info", admin_id=message.from_user.id, details=f"Категория: {category}, ID: {msg_id}")
    await state.clear()
...
async def cb_fuck_toggle(callback: CallbackQuery):
    ...
    await replace_menu(callback, "Настройка функции 'Ебать 1x4':", fuck_menu_keyboard())
    await add_log("fuck_toggle", admin_id=callback.from_user.id, details=f"Статус: {'включена' if new == '1' else 'выключена'}")
...
async def process_fuck_trigger(message: Message, state: FSMContext):
    ...
    await add_log("fuck_set_trigger", admin_id=message.from_user.id, details=f"Триггер: {trigger}")
    await state.clear()
...
async def process_fuck_reply(message: Message, state: FSMContext):
    ...
    await add_log("fuck_set_reply", admin_id=message.from_user.id, details=f"Ответ: {reply}")
    await state.clear()
...
async def handle_fuck_1x4(message: Message):
    ...
    await add_log("fuck_trigger", user_id=user.id, details=f"Ответ: {final_reply[:50]}...")
...
async def cb_destroy_yes(callback: CallbackQuery):
    ...
    try:
        ...
        await add_log("destroy_group", admin_id=callback.from_user.id, details=f"Кикнуто участников: {kicked}")
    except Exception as e:
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await add_log("clear_history", admin_id=callback.from_user.id, details=f"Категория: {category}")
