...
LOG_GROUP_ID = int(os.getenv("LOG_GROUP_ID", -5578822554))
...
class SupportForm(StatesGroup):
    waiting_application = State()
class ConfirmApplication(StatesGroup):
...
async def log_action(action_text: str):
    try:
        await bot.send_message(LOG_GROUP_ID, f"📝 {action_text}")
...
# ---------- Работа со списками сообщений заявок ----------
...
async def assign_flood_role(user_id: int, role_name: str, emoji: str):
    ...
    await log_action(f"Одобрена заявка во флуд для user_id {user_id}")
...
async def assign_rest_role(user_id: int, role_name: str, expiry_date: str):
    ...
    await log_action(f"Одобрена заявка в рест для user_id {user_id}")
...
async def assign_staff_role(user_id: int, role_name: str, username: str):
    ...
    await log_action(f"Одобрена заявка в стафф для user_id {user_id}")
    return True
...
# ---------- Функция очистки просроченных pending (4 дня) ----------
async def cleanup_expired_pending():
    now = datetime.now()
    threshold = now - timedelta(days=4)
    threshold_iso = threshold.isoformat()
    async with aiosqlite.connect("bot.db") as db:
        for table in ["flood_applications", "rest_applications", "staff_applications", "support_applications"]:
...
async def set_cooldown(user_id: int, category: str, seconds: int):
    ...
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(f"Выдан кд {category} для user_id {user_id} на {seconds} сек")
...
async def remove_cooldown(user_id: int, category: str):
    async with aiosqlite.connect("bot.db") as db:
        ...
        await db.commit()
    await log_action(f"Снят кд {category} для user_id {user_id}")
...
async def set_cooldown_duration(category: str, seconds: int):
    async with aiosqlite.connect("bot.db") as db:
        await db.execute("INSERT OR REPLACE INTO cooldown_settings (category, duration_seconds) VALUES (?, ?)",
                         (category, seconds))
        await db.commit()
    await log_action(f"Изменена длительность кд {category} на {seconds} сек")
...
# ---------- Вспомогательная функция для начала подтверждения ----------
async def start_confirmation(message: Message, category: str, text: str, state: FSMContext):
    await state.update_data(category=category, text=text, edit_count=0)
    await message.answer(
        "Вы уверены что хотите подать заявку?\n"
        "Напишите \"да\" или \"нет\" или \"редактировать\""
    )
    await state.set_state(ConfirmApplication.waiting_confirmation)
...
async def is_user_in_application_process(state: FSMContext) -> bool:
    ...
    return current_state in [
        ...
        SupportForm.waiting_application.state,
        ConfirmApplication.waiting_confirmation.state,
        EditApplication.waiting_new_text.state
    ]
...
async def process_flood_application(message: Message, state: FSMContext):
    ...
    await start_confirmation(message, "flood", text, state)
...
async def process_rest_application(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        ...
        return
    text = message.text or ""
    await start_confirmation(message, "rest", text, state)
...
async def process_staff_application(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        ...
        return
    text = message.text or ""
    await start_confirmation(message, "staff", text, state)
...
async def cb_support(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SupportForm.waiting_application)

@dp.message(SupportForm.waiting_application)
async def process_support_application(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        await message.answer("Отправка обращения отменена.")
        await state.clear()
        return
    text = message.text or ""
    await start_confirmation(message, "support", text, state)

# ---------- Обработчик подтверждения ----------
@dp.message(ConfirmApplication.waiting_confirmation)
async def process_confirmation(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == "да":
        ...
        if category == "flood":
            ...
            table = "support_applications"
            status = "pending_support"
        else:
            await message.answer("Неизвестная категория, обратитесь к администратору.")
            await state.clear()
            return
        ...
        confirm_msg = await message.answer("Ваша заявка отправлена. Ожидайте решения.")
        # Сохраняем message_id подтверждения для отмены
        async with aiosqlite.connect("bot.db") as db:
            await db.execute(f"UPDATE {table} SET confirm_message_id=? WHERE user_id=?", (confirm_msg.message_id, user_id))
            await db.commit()

        if category == "support":
            # Отправляем заявку в админ-чат
            admin_msg = await bot.send_message(
                ADMIN_INFO_CHANNEL_ID,
                f"📩 Новая заявка поддержки\nUser ID: {user_id}\nТекст: {text}"
            )
            async with aiosqlite.connect("bot.db") as db:
                await db.execute("UPDATE support_applications SET support_message_id=? WHERE user_id=?",
                                 (admin_msg.message_id, user_id))
                await db.commit()
            await log_action(f"Подана заявка поддержки от user_id {user_id}")

        await log_action(f"Подана заявка ({category}) от user_id {user_id}")
        await state.clear()
        ...
        await message.answer("Выберите раздел:", reply_markup=main_menu_keyboard(await get_user_status(user_id), await is_user_in_flood_group(user_id)))

    elif message.text and message.text.lower() == "редактировать":
        if edit_count >= 10:
            await message.answer("Превышено количество редактирований (10). Возврат в главное меню.")
            await state.clear()
            await message.answer("Выберите раздел:", reply_markup=main_menu_keyboard(await get_user_status(user_id), await is_user_in_flood_group(user_id)))
            return
        await state.update_data(edit_count=edit_count + 1)
        await message.answer("Отправьте новую анкету:")
        await state.set_state(EditApplication.waiting_new_text)
    else:
        await message.answer("Пожалуйста, напишите \"да\", \"нет\" или \"редактировать\".")

# ---------- Обработчик редактирования ----------
@dp.message(EditApplication.waiting_new_text)
async def process_edit_application(message: Message, state: FSMContext):
    if message.text and message.text.lower() == "отмена":
        await message.answer("Подача заявки отменена.")
        await state.clear()
        await message.answer("Выберите раздел:", reply_markup=main_menu_keyboard(await get_user_status(message.from_user.id), await is_user_in_flood_group(message.from_user.id)))
        return
    text = message.text or ""
    ...
    category = data.get("category")
    edit_count = int(data.get("edit_count", 0))
    await state.update_data(text=text, edit_count=edit_count)
    await message.answer(
        "Вы уверены что хотите подать заявку?\n"
        "Напишите \"да\" или \"нет\" или \"редактировать\""
    )
...
# ---------- Отмена после отправки (reply на подтверждение) ----------
@dp.message(F.reply_to_message)
async def cancel_after_send(message: Message):
    if not message.text or message.text.lower() != "отмена":
        return
    user_id = message.from_user.id
    reply_msg = message.reply_to_message
    if not reply_msg.text or "Ваша заявка отправлена. Ожидайте решения." not in reply_msg.text:
        return

    # Проверяем все таблицы заявок
    async with aiosqlite.connect("bot.db") as db:
        for table in ["flood_applications", "rest_applications", "staff_applications", "support_applications"]:
            async with db.execute(f"SELECT confirm_message_id, status FROM {table} WHERE user_id=?", (user_id,)) as cur:
                row = await cur.fetchone()
                if row and row[0] == reply_msg.message_id and row[1] == 'pending':
                    await db.execute(f"DELETE FROM {table} WHERE user_id=?", (user_id,))
                    await db.execute("UPDATE users SET status='none' WHERE user_id=?", (user_id,))
                    await db.commit()
                    await message.answer("Ваша заявка отменена.")
                    await log_action(f"Отменена заявка ({table.split('_')[0]}) от user_id {user_id}")
                    return
    await message.answer("Заявка не найдена или уже обработана.")

# ---------- Новый обработчик: мои кд ----------
...
async def cb_admin_app_support(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    await show_applications(callback.message.chat.id, "support_applications", "Заявка в поддержку")
...
async def process_ban(message: Message, state: FSMContext):
    ...
    await log_action(f"Бан user_id {target_id} админом {actor_id}")
    await state.clear()
...
async def process_unban(message: Message, state: FSMContext):
    ...
    await log_action(f"Разбан user_id {target_id} админом {actor_id}")
    await state.clear()

# ---------- Админ: переключение заявок (только владельцы) ----------
...
async def toggle_app_setting(setting_key: str, info_key: str, text_on: str, text_off: str):
    ...
    await log_action(f"Изменено состояние заявок {setting_key}: {'закрыты' if new == '0' else 'открыты'}")
    return new
...
# ---------- Админ: настройки (только владельцы) ----------
...
async def process_flood_settings(message: Message, state: FSMContext):
    ...
    await log_action(f"Обновлены роли флуда пользователем {message.from_user.id}")
    await state.clear()
...
async def process_staff_settings(message: Message, state: FSMContext):
    ...
    await log_action(f"Обновлены роли стаффа пользователем {message.from_user.id}")
    await state.clear()
...
async def process_delete_flood_role(message: Message, state: FSMContext):
    ...
    await log_action(f"Удалена роль флуда '{role_name}' админом {message.from_user.id}")
    await state.clear()
...
async def process_delete_staff_role(message: Message, state: FSMContext):
    ...
    await log_action(f"Удалена роль стаффа '{role_name}' у user_id {user_id} админом {message.from_user.id}")
    await state.clear()
...
async def process_delete_rest_role(message: Message, state: FSMContext):
    ...
    await update_rest_info()
    await message.answer(f"Роль '{role_name}' удалена.")
    await log_action(f"Удалена роль рест '{role_name}' админом {message.from_user.id}")
    await state.clear()

# ---------- Админ: управление админами (только владельцы) ----------
...
async def process_add_admin(message: Message, state: FSMContext):
    ...
    await log_action(f"Назначен админ user_id {user_id} владельцем {message.from_user.id}")
    await state.clear()
...
async def process_remove_admin(message: Message, state: FSMContext):
    ...
    await log_action(f"Удалён админ user_id {user_id} владельцем {message.from_user.id}")
    await state.clear()

# ---------- Админ: написать сообщение (поддержка медиа) ----------
...
async def process_send_message(message: Message, state: FSMContext):
    ...
    await log_action(f"Отправлено сообщение в {target} админом {message.from_user.id}")
    await state.clear()
...
async def process_delete_message(message: Message, state: FSMContext):
    ...
    await log_action(f"Удалено сообщение в {target} админом {message.from_user.id}")
    await state.clear()
...
async def process_ban_app(message: Message, state: FSMContext):
    ...
    await log_action(f"Запрет на заявки {category} для user_id {user_id} админом {message.from_user.id}")
    await state.clear()
...
async def cb_destroy_yes(callback: CallbackQuery):
    ...
    try:
        ...
        await log_action(f"Уничтожение группы флуда выполнено специальным владельцем {callback.from_user.id}")
    except Exception as e:
...
async def admin_reply(message: Message):
    ...
    # Проверка ответа на заявку поддержки (админ отвечает на сообщение в админ-чате)
    if chat_id == ADMIN_INFO_CHANNEL_ID:
        async with aiosqlite.connect("bot.db") as db:
            async with db.execute("SELECT user_id FROM support_applications WHERE support_message_id=?", (msg_id,)) as cur:
                row = await cur.fetchone()
                if row:
                    user_id = row[0]
                    # Пересылаем ответ пользователю
                    try:
                        await bot.copy_message(chat_id=user_id, from_chat_id=chat_id, message_id=message.message_id)
                        await message.answer("Ответ отправлен пользователю.")
                        await log_action(f"Админ {message.from_user.id} ответил на заявку поддержки user_id {user_id}")
                    except Exception as e:
    ...
    if "Заявка во флуд" in original_msg.text:
        ...
        table = "staff_applications"
    elif "Заявка в поддержку" in original_msg.text:
        table = "support_applications"
    else:
        return
    ...
    if text.lower().startswith("принять"):
        ...
        if table == "flood_applications":
            ...
            if await assign_staff_role(applicant_id, role_name, username):
                ...
                return
        elif table == "support_applications":
            # Для поддержки просто помечаем как решённую
            new_status = "approved"
            # Отправить уведомление пользователю?
            try:
                await bot.send_message(applicant_id, "✅ Ваше обращение в поддержку рассмотрено.")
            except:
                pass
        ...
        await log_action(f"Принята заявка ({table}) от user_id {applicant_id} админом {message.from_user.id}")
        ...
        await log_action(f"Отклонена заявка ({table}) от user_id {applicant_id} админом {message.from_user.id}")
        # Уведомление пользователю
        try:
            await bot.send_message(applicant_id, "❌ Ваша заявка была отклонена.")
        except:
            pass
    else:
...
async def cb_clear_history_category(callback: CallbackQuery):
    ...
    await log_action(f"Очищена история заявок {category} админом {callback.from_user.id}")
